...
## 3.84 Object Library live preview: lighting, framing and the double object — 2026-09-22

Lee's four reports on the 3.83 preview. Three of them turned out to be one bug.

### ★★★ Defects 1 and 2 are the same bug, and it is not "too dim"

> "the lighting is directly influenced by the current levels Lighting Color and Ambient Color"
> "there is no point light to illuminate the camera facing side of the preview"

A character rendered as a **pure black silhouette** against its correctly-lit orange backdrop.

What the measurements said before any theory:

| | |
|---|---|
| exposure 64 | backdrop blows out to white, object **still pure black** |
| thumb light intensity 5000 | **no change at all** |
| light world position | `world == local`, correct, 296 units from the object |
| light type / range | POINT / 2900 — comfortably reaching |
| scene ambient | `(0.000, 0.000, 0.000)` — confirmed, not assumed from the UI panel |

So it receives **exactly zero light**, and no amount of intensity or exposure was going to change
that. Root cause:

```
wiRenderPath3D.cpp:473   UpdatePerFrameData(*scene, visibility_main, frameCB, ...)
wiRenderer.cpp:6349      for (uint32_t lightIndex : vis.visibleLights)
```

**The shader's light array is packed once per frame from the MAIN camera's cull.**
`ComputeTiledLightCulling(res, vis, ...)` touches the per-camera visibility only for an
"everything is empty" early-out (`wiRenderer.cpp:12228`) — its compute shader reads that one
shared array out of the FrameCB. The preview's lights sit at the preview camera, ~39,000 units
above the editor camera and ~85° off its view axis, so `UpdateVisibility` culled them from
`visibility_main`, they were never written into the entity array, and **no camera could see them —
including the preview's own**.

DX11 never hit this because `ForceRender` ran a whole extra `Update()` + `Render()` with GG camera
0 already moved to the preview pose, so `UpdatePerFrameData` repacked the array with the preview
camera. That is not portable: it is a second full frame per hover.

★ **A per-camera light count is not a count of lights the shader can see.** `DUMP_OBJPREVIEW`
reported `visLight=2` for the entire time the object rendered pure black — the preview camera's own
cull was perfectly happy. The dump now separates `visLightCulled` (that number, kept because it is
still worth seeing) from `litLights` and `mainLights`, which are the ones that mean anything.

**Fix** — `GGObjectPreview_PreVisibility()`, called from `Master::Update` immediately after
`WickedCall_UpdateCharacterShadows()`: the same slot, for the same reason, as the call it sits next
to — after `Scene::Update` rebuilds `scene.aabb_lights` and before `PreRender`'s `UpdateVisibility`
reads them. It gives the two preview lights an infinite cull AABB so they always survive the main
cull. Costs two entity slots and nothing else: they are 39,000 units up with a range that reaches
only the preview, so they illuminate nothing in the level. **No engine change.**

**Defect 2 proper** — the rig now scales with the framing distance. `intensity = k·d²` and
`range = 6d`, so one constant frames a 15-unit prop and a 2500-unit building identically; the
shipped 8 cd / 2900 range was a fraction of a lux on a small prop and entirely out of range on a
large one (`fCamMove = fLargest * ~3.5` runs from ~50 to ~8750 units). Key is lifted and pushed to
one side of the camera so the camera-facing side is modelled rather than flat-on; fill sits opposite
at 0.45×.

**Strength matched to the DX11 reference (3.84b, Lee: "the light is insanely brighter").** The shipped
static thumbnails were generated by DX11, so one sitting in the same grid IS the reference - and the
statistic is the whole story:

| | mean | p90 | **p99** |
|---|---|---|---|
| DX11 static thumbnail | 66.8 | 106.0 | **145.6** |
| DX12 live, k = 0.0006 | 67.6 | 112.0 | **220.1** |

★★ **On the MEAN the two are indistinguishable** - 66.8 against 67.6 - because the backdrop plane
fills most of the cell and swamps the subject. On **p99**, the brightest 1%, which is the lit side of
the subject, the DX12 highlight is **51% hotter**. Third time this campaign that the frame-wide
average hid the signal (see the reflection fix, where a >40 threshold exceeded the puddle's entire
dynamic range). Measure the SIGNAL, not the frame.

`tools/objpreview_match_dx11.sh` is self-calibrating: every run measures the static thumbnail in the
same cell, same resolution, same session, so the target travels with the run. Sweep against reference
p99 150.3:

| k | 0.00008 | 0.00013 | 0.00015 | 0.00025 | 0.00040 | 0.00060 |
|---|---|---|---|---|---|---|
| p99 | 132.5 | **147.8** | 155.4 | 183.1 | 208.2 | 223.9 |

Shipped at **k = 0.00013**, p99 147.8 against 150.3 - within 1.7%. The k = 0.00060 row reproducing
Lee's own 220.1 to within 2% is what says the measurement chain is sound.

### Defect 3 — the camera was too close, by exactly 1.39×

> "the camera seems to be a little too close causing the previewed object/character to appear larger"

`GrabBackBufferCopy` passed the **level's** `t.visuals.CameraFOV_f`. DX11 rendered thumbnails with
the **editor's fixed 45** (`M-GridEditB_part3.cpp:213` sets `fUsedFOV = 45` whenever
`bImGuiInTestGame` is false), and every distance constant in that function — `fAdjustRange`,
`fCamMove`, the `fLargestY` ladder — was fitted against it. Now fixed at 45; measured `fov` 0.234 →
0.324.

★ Third instance in two days of the same mistake: **the preview is its own scene, and every
parameter it inherited from the level was one that could be wrong for it** — exposure (3.83), near
and far planes (3.83), and now the field of view.

### Defect 4 — two objects superimposed

`GGObjectPreview_ParkObject` adopted a new object without giving the previous one back, so both sat
at the park point and the preview camera drew both. Split the restore out of `Stop()` into
`GGObjectPreview_RestoreParked()` and call it when the parked id changes; `Stop()` now uses it too.

★ Why it was only "sometimes": moving between cells normally crosses a **gap** where nothing is
hovered, which ends the preview and un-parks cleanly. It is the **fast flick between adjacent
thumbnails** that reproduces it. Observed `evict=1` on exactly that transition, `evict=0` when the
pointer passed through a gap — both correct.

Also fixed while in there: `WickedCall_EnableThumbLight(false)` was nested inside the backdrop
branch of `Stop()`, so a preview that never got a backdrop left both lights — and the light-shaft
and lens-flare switches they turn off, which are **global RenderPath3D state** — stuck for the rest
of the session. Now unconditional.

### Measurement notes

★★ **Bracket the knee before reading a flat line as "no effect".** The light response saturates
hard: at d=296, absolute intensity 150 is already washed out and everything above ~1200 is
pixel-identical. My first sweep ran k = 0.02 → 6.0, which is 1752 → 525,000 candela — the entire
sweep sat on the flat top, so a working knob read as a broken one and I nearly went looking for a
second bug. The usable range was two orders of magnitude *below* where I started.

★ **Eyeballing crops is not measuring them.** Four crops looked identical to me; a mean-luminance +
blown-pixel count over the cell separated them immediately, and later showed the real curve
(20 → 0.0% blown, 50 → 0.2%, 150 → 2.5%, 600 → 6.6%). `k = 0.0006` was picked off that curve,
against the Astra character at d=296 (intensity 50).

⚠ **The heredoc trap again**, twice. `sed 's|\\|/|g'` inside a `<< 'EOF'` heredoc arrives as
`s|\|/|g` and dies with "unterminated `s' command" — the probe took its screenshots and silently
failed to keep any of them. Write probe scripts to a FILE, exactly as the rule already says.
