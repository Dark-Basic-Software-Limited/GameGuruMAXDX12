...
| 1.59(+b) Delayed-cascade stagger + far-cascade cull dedicated-aware; slot projection-Z = culling-Z | applied 2026-07-30 (`b4fc81ed` + **1.59b `ef719b45`: dedicated slots' PROJECTION depth range now matches their CULLING extension — was ext×4 ≈ 12ft vs ~83ft culling, so up-sun geometry (terrain banks/monuments) depth-CLAMPED to the near plane and stamped FALSE full-occlusion rectangles across the slot's ground footprint (user repro: dark rectangular strips on sand near characters, immune to receiver bias + grass removal — convicted by elimination + before/after diff). Sun cascades unchanged; FPS unchanged**, wiRenderer.cpp: DelayedShadowCascadeState.dedicatedCount field; stagger group offset past dedicated slots — dedicated + sun cascade 0 every frame, sun 1..N stagger; dedicated-count CHANGE forces full refresh so frozen per-slot matrices never cross shifted indices; far-cull thresholds keyed on sun_cascade = raw slot − dedicated_count, dedicated slots exempt), lib rebuilt | Both features previously BAILED entirely whenever character dedicated shadows were live (slots shift cascade indices) — measured ~70→55 FPS with one hero slot at the grass-heavy fighter view. After 1.59: dedicated ON 69.3-69.6 vs OFF 69.7 FPS — **the dedicated character shadow now costs ~0.3 FPS**, with DELAYED_SHADOWS confirmed staggering while the slot refreshes every frame. Crispness verified intact post-fix |
