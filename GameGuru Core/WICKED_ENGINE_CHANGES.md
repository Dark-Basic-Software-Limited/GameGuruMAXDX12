...
| 1.49 Grass strand LOD (far-distance decimation + width compensation) | applied 2026-07-26 in Wicked commit `8209a359` (ShaderInterop_HairParticle.h padding fields, hairparticle_simulateCS.hlsl, wiHairParticle.cpp), lib rebuilt — **default OFF (visual trade, user decides)** | `wi::gg_grass_lod` + step2/step4 viewDistance fractions + width boost. In the simulate CS: beyond step2 only every 2nd strand appends to the culled index buffer, beyond step4 every 4th — selection = hash(strand id) + camera distance ONLY, so the pattern is STABLE at a parked camera (no temporal pops — the 1.37 lesson); survivors' billboard width ×boost per step preserves screen coverage; dropped strands take the existing 1.25 early-out (sim work saved too). Draw savings flow to EVERY pass reading the culled buffer (prepass/opaque/shadow). GG grass systems only (`grass_type != 0`), character hair untouched. MEASURED TESTPRO1 gate view (clean steady-state ABAB ×2 passes): **72.1 → 77.2 FPS (+0.9ms) at 0.35/0.60 ×1.8** (first-load A/B showed up to 79-81; ~+1ms is the conservative steady-state figure); screenshots indistinguishable at the test camera; clean live revert. Harness `SET_GRASSLOD <0|1> [f2 f4 boost]` |
