...
| **1.72 Grass draw-distance fade (hairparticle_simulateCS) + game-side density rings that follow the slider** | applied 2026-08-01 (engine: `hairparticle_simulateCS.hlsl` tapers `strand_length` to zero across the outer 12% of `xHairViewDistance`, before the visibility test. Game: `GGTerrainWicked.cpp` ProcessGrassChunks scales tier3C/tier2C by the slider relative to its own default, clamped 0.75-2.0 / up to 3.0 chunks; harness `SET_GRASS drawdist`) | **User report: "moving from 750 to 7000 does not change how far grass renders".** Measured on Island Showdown: 750 and the 1500 default produced an IDENTICAL 34 systems / 3.40M strands (lowering the slider freed nothing, because the rings were hard-coded at 1.5/2.2 chunks and `nearC = min(1.5, outerC)` with outerC never below ~1.6), while 7000 added 54 chunk systems at the 5%/18% tier densities — ~1 blade per 3.6 m² over a 134 m chunk, invisible past 200 m, but each still paying a full simulate dispatch (87 → 56 FPS for two pixel-identical screenshots). Now: rings scale with the slider anchored at the default, so **1500 is bit-identical** (same 34/3.40M, screenshot delta under the animation noise floor) while **750 → 1.35M strands and −1139 MB driver VRAM** and 7000 spends its budget on full-density grass (5.36M strands) instead of invisible shells. The fade lives in the CS rather than a pixel shader so it applies identically to colour, prepass and shadow passes with no alpha blending, overlapping the existing 20% dither ramp in hairparticleVS. Caveat recorded in VRAM_CENSUS.md: the visible-range gain could not be screenshotted — every demo tried paints grass in near-field patches, and a same-setting control diff matched the 750-vs-7000 diff exactly (all grass sway). |
