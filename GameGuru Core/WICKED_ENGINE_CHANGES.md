...
| 1.53 Terrain VT tiling share-mips (near-camera scale-swim fix) | applied 2026-07-28 (ShaderInterop_Renderer.h `TerrainVirtualTexturePush.padding0` → `gg_tile_share`, terrainVirtualTextureUpdateCS.hlsl policy helper across all 4 map variants, wiTerrain.cpp knob + push fill; game: `GGTerrainWicked_SetTileShare` + harness `SET_TERRAINTILE`), lib rebuilt — **default K=1** | **User-reported: painted terrain shows overlapping blends of the same texture at different UV scales, transitioning right at the camera ("swimming mess underfoot when walking").** Mechanism: stock bakes each VT mip of a chunk region with the texture repeat count HALVED per mip (`repeats = region_res/texture_dim`, floored at 1) — deliberate anti-tiling for distance — but adjacent VT mips then hold DIFFERENT tilings, and `SampleVirtual`'s custom trilinear (`lerp(mipN, mipN+1, frac(lod))`) cross-fades two UV scales of the same texture; in MAX's inch-scale world the texel density per real metre is a fraction of the metre-scale design assumption, so the first cross-fade bands sit metres from the camera (user's meter-vs-inch prediction: correct). Policy: **the K mips below mip0 keep mip0's repeat count** — those bakes become true downsamples of one layout (invisible trilinear transitions near); beyond K the halving resumes (one gentle 2× cross-fade per boundary, pushed K mips out; far-field anti-tiling preserved). K=0 = stock bit-for-bit. Live-tunable: `SET_TERRAINTILE <K>` sets `wi::terrain::gg_terrain_tile_share_mips` + queues the proven-safe 1.13 fast repaint on all resident chunk VTs (visible in ~a frame; knob-time repaint burst only — bake-time policy, ZERO per-frame cost). Verified A/B from the user's snowy test scene: K=0 = mixed-scale patchwork behind the player marker; K=1 = uniform near field; K=3 = one crisp scale across the whole view. AWAITING USER TASTE-TUNE of the default |
