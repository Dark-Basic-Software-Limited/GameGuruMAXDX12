#!/bin/bash
# Measure POLYS at increasing soak times. If the count CLIMBS to the reference, the sweep's low
# reading was an unsettled scene on a tired machine, not lost geometry.
...
echo "  $DEMO  (reference $REF)"
for t in 15 30 60 90; do sleep 15; echo "    +${t}s : $(polys)"; done
