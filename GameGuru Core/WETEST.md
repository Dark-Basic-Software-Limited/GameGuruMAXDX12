...
| `GPUP_REGEN` | (none) | Re-create `texNoiseOrig`/`texDist2` from their PNGs on the LIVE instance (safe anywhere; deferred-destroy covers in-flight frames). The white-out (#120) confirm/refute lever: these are the only gpup GPU resources with process lifetime, consumed by every emitter every sim tick — **on a caught white-out, REGEN clearing the screen confirms input-texture poisoning** (durable fix = recreate at level load); no effect on a healthy scene |
