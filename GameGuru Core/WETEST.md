...
| `MOUSE_AT` | `<x> <y>` | GGMAX 3.83: move the REAL OS cursor to those CLIENT pixels of the MAX window (same space as `PICK_AT` and a `SCREENSHOT`). The only way to drive anything hover-triggered - `ImGui::IsItemHovered()` reads `io.MousePos`, which comes from the cursor, so `CLICK` and `PRESS_KEY` cannot reach it |
| `MOUSE_DOWN` / `MOUSE_UP` | (none) | GGMAX 3.83: press / release the left button at the current cursor position. ⚠ **Use these, not `MOUSE_CLICK`, on any ImGui widget** - a down+up pair inside one `SendInput` is drained by ONE message pump and `io.MouseDown[0]` nets to false, so ImGui never sees a click. Pattern: `MOUSE_AT x y` → sleep → `MOUSE_DOWN` → sleep → `MOUSE_UP` |
| `MOUSE_CLICK` | `[<x> <y>]` | GGMAX 3.83: move (optional) and click in one call. Works on native windows; see the warning above for ImGui |
| `DUMP_OBJPREVIEW` | (none) | GGMAX 3.83: every link in the Object Library live-preview chain in one line - GG-side request, camera entity, engine render target, LDR copy, ImGui descriptor, preview visibility counts, exposure, clip range, backdrop state and distance. Tells a preview that was never requested from one never rendered from one never bound |
| `SET_OBJPREVIEW` | `<0..3> [exposure] [backdrop]` | GGMAX 3.83: stage the live preview so a device hang names which step did it - 0 off, 1 engine `render_to_texture` only, 2 + tonemap, 3 + ImGui bind (default). `exposure` (default 1.0, <=0 = use the level's) and `backdrop` (0 leave, 1 opaque+keep texture, 2 opaque+flat colour) do NOT restart the pass, so they can be swept against one live hover |
| `SET_OBJPREVIEW_LIGHT` | `<k> [absIntensity] [fovDeg]` | GGMAX 3.84: the preview's own key/fill rig. `k` is the illuminance landed on the subject - intensity is `k * d^2`, so one value frames a 15-unit prop and a 2500-unit building identically. Default 0.0006. `absIntensity` >0 overrides with a fixed candela value (bisecting only). `fovDeg` is the framing FOV, default 45 = the editor's fixed thumbnail FOV. None of them restart the pass, so all three sweep against ONE live hover. ⚠ The light response SATURATES HARD: at d=296, 150 absolute is already washed out and everything above ~1200 is pixel-identical, so a sweep that starts too high reads as a dead knob |
...
| `DUMP_GPUGAPS` | (none) | GGMAX 2.94c: names WHERE the GPU dead time is - the biggest holes between measured ranges in the latest frame, each labelled with the range that closed before it and the range that opens after it. Needs `ENABLE_PROFILER`. Read it on a PARKED camera (latest frame, not averaged). This is what found the 3.96 ms `TerrainVT - WritebackTileRequests` |
...
| `SET_TREES pool` | `<N>` | ⚠ **READ THIS BEFORE USING IT FOR AN A/B.** Sets `g_treePoolSize`, which since GGMAX 2.19 is the pool **CEILING**, not its size. **Raising it** now permits further lazy growth on the next frame that wants more slots. **Lowering it does NOT shrink an existing pool** — slots are only released by `GGTrees_WickedShutdown`, whose caller has zero callers, so they live for the process. Before 2.19 this command was a complete no-op after startup and that invalidated a whole A/B: the 2026-08-09 "the tree pool costs nothing" result ran both arms on the identical 6000 slots, and the unchanged `SCENE_OBJECTS` was the proof the knob did nothing rather than proof the pool was free (SWITCHESCAPE_PERF.md §2). **For a real reduction use `setup.ini treepool=<N>`**, read in `GetSetupIniEarly()` before the pool can grow at all. ★ RULE: prove a knob REACHES the thing before trusting its A/B |
...
| `SET_SINGLEQUEUE` | `0` or `1` | A/B Wicked delta 1.48b: route COMPUTE/COPY command lists onto the GRAPHICS queue + drop same-queue fences (batches 15→1, deps 12→0). ★ **NOW DEFAULT 1 (engine 2.17).** The old NEGATIVE verdict was stale. Full 19-demo hub sweep, three arms each, 40 s settle-gated arms: **18/19 positive, mean +5.07%, POLYS bit-identical on all 19, no tail regression** (best Trapped +16.7%, Switch Escape +15.8%, Zombie Cellar +11.9%; only negative Horseshoe Bend −0.5%, inside drift). Persistent revert `setup.ini singlequeue=0`; live revert `SET_SINGLEQUEUE 0`. (Was −4.7 FPS on TESTPRO1 on a much older engine; I expected a light-vs-heavy crossover, tested a heavy level for it, and there isn't one.) Why it wins: the editor frame is GPU-FENCE-bound — CPU waits on the fence in 99.4% of frames — and ~1 ms of GPU frame is cross-queue bubble at `deps=9`. The Island Showdown max-stall outlier that blocked this (3.37→6.71 ms on a 22 s arm) did NOT reproduce over 40 s arms — it was a lazy-PSO compile. Data: `tools/singlequeue_sweep_0809_full.txt`, criteria in `SWITCHESCAPE_PERF.md` §8b. Judge it with `SET_SUBMITSTATS 1` + `SUBMIT_STALL_WINDOW`, not the snapshot below. The 1.48a `SUBMIT_PHASES_MS` readout in GET_PERF_DATA shows close/fences/present/sync/stall + lists/batches/deps — the 'submit tail' is ~95% the end STALL (GPU still busy) = the frame is GPU-WALL-bound |
...
| `DUMP_ENVPROBE` | `[radius=3000]` | (2.73, #155) Dump every env probe cube + the raw sky cube as `.dds` next to the exe's `Files/` (`envprobe_<i>_<name>.dds`, `envprobe_sky.dds` — decode offline with texconv) plus `envprobe_dump.txt`: per-probe entity/name/pos/range/res/mips/dirty/realtime, weather.skyMapName, and a census of all objects within radius of probes[0] (the global/base env reflection). **Architecture this instrument revealed**: base env reflection = `probes[0]` = GGTerrain's `globalEnvProbe`, a 128px 4-mip BC6H capture at (0, terrain-height, 0); the raw skybank `_cube.dds` is NEVER sampled for reflections (mip-0 sky backdrop only); the 8 `localEnvProbe` pool slots baked ONCE at engine init (pre-level twilight with a moon-like blob) until 2.73 re-bakes them per level load |
| `SET_DEBUGPROBES` | `<0\|1> [scale]` | (2.74b/2.75) Force the engine's debug env-probe mirror spheres on/off without a picked probe marker; optional `scale` = sphere radius in world units (engine 2.75 knob, default 1; the editor pick path auto-sizes it to the marker ball × 1.15). ⚠ A camera INSIDE the radius sees nothing (backface cull) — stand OUTSIDE the scale distance (lighting_loop otherwise clears the flag every frame; this is a sticky override). The debug sphere is the TRUE data view of a probe cube. ⚠ **The big ball a user sees when selecting a probe marker is the `%probe` ENTITY (probe.dbo, scale 50, selection outline) — NOT this debug sphere**: its per-face "porthole" look is a property of that entity's PBR shading, while the underlying cube map is seamless (proof chain in §2.74b: DUMP_ENVPROBE → panorama reprojection → offline mirror-ball sim → this debug sphere, all clean) |
| `SET_PROBEVIEW` | `<mode> [mip] [scale]` | (2.89, #157) **PROBE INSPECTION MODE.** Re-points the engine's debug mirror sphere: `1` = show the GLOBAL cube (`probes[0]`, exactly what shaders read as `GetScene().globalprobe`), `2` = show the LOCAL cube at the same spot, `0` = off/stock. `mip` picks the level sampled (0 = raw capture, higher walks the filtered chain) — the visible receipt that the mip plumbing works is mip 3 rendering as a uniform blur. `scale` = sphere radius when no marker is picked (default 60). Draws exactly ONE sphere: on the picked marker if there is one, else hovering 1.5 radii in front of the camera. ini: `probeview` / `probeviewmip` (INT passthrough). ⚠ This sphere is a RAW MIRROR (`cubeMapPS`: no Fresnel, no roughness, **no parallax**) — it shows cube CONTENT and by design CANNOT show read-path defects. Judging a reflection defect needs a real PBR surface; that mistake cost most of the 08-17 night |
| `SET_PROBEPARALLAX` | `<0\|1\|2\|3>` | (2.89, #157) **The circles fix.** Env-probe parallax precision. `0` = stock `half` math, which overflows fp16 (65504) for any probe OBB over 37,820 units and REPRODUCES the defect; `1` = float math with parallax kept (the precision-only fix); `2` = float + MAGENTA on every pixel the stock math would break (the overflow map — on Lee's test scene this covers the whole terrain); **`3` = float + skip parallax entirely on level-sized boxes (DX11-style raw reflection vector) — the shipping DEFAULT**, chosen by Lee 08-18 after the A/B: parallax against a box enclosing the whole level only drags the reflected horizon by the surface's offset from the map origin, so the look becomes position-dependent for no physical gain. ini: `probeparallax` (INT passthrough). Reports the live box half-extent and worst-case exit distance so the maths can be checked against the picture |
| `SET_PROBEONLYGLOBAL` | `<0\|1>` | (2.89, #157) `1` = the shader ignores every LOCAL probe so all surfaces read the GLOBAL cube. Shader-side, so GGTerrain's per-frame pool tracking cannot undo it. This is how the marker ball's circles are reproduced on demand: the ball normally sits inside its own local probe box (clean) and only falls through to the global cube while a drag releases the pool slot — the difference between the "clean ball" and "circles ball" screenshots |
| `SET_GLOBALPROBEBOX` | `<halfextent>` | (2.89, #157) Resize the GLOBAL probe's parallax OBB live (stock 50000). Threshold experiment for the fp16 story: with `SET_PROBEPARALLAX 0` the stock math is in range below 65504/√3 = 37,820 and overflows above it. Box only — does NOT re-capture the cube. ⚠ GGTerrain re-asserts the box on its own refresh path (height change / level load), so treat it as a transient probe |
| `REFRESH_ENVPROBE` | (none) | Editor-identical full probe refresh: sets `g_bLightProbeScaleChanged` (lighting_loop → ClearEnvProbeList → global probe re-render + marker re-add) AND `WickedCall_UpdateProbes()` (the 2.73 pool re-bake path). Pair with DUMP_ENVPROBE before/after to separate "stale capture" from "reproducibly corrupt capture" |
| `DUMP_TERRAINSURF` | `[x z]` | (2.74) Save the LIVE SVT atlases (basecolor + surface) as `terrain_atlas_*.dds` next to `Files/` + `terrainsurf_dump.txt` with the nearest terrain chunk material's per-slot texture/residency descriptors. Decode offline (texconv; sparse readback = unmapped tiles read black). ⚠ **Terrain-roughness A/B lessons this instrument settled (#155)**: Island Showdown's visible beaches are mat8/mat18-class (roughness already ~250) so whole-frame diffs read as noise even with mat2 swapped to chrome; a dielectric chrome patch is INVISIBLE top-down (F≈f0≈0.5% at normal incidence) — grazing view or nothing; and `SET_CAMERA` yaw must be proven to face the target (two "null" A/Bs were cameras facing away). The conclusive recipe: PAINT_TEST a mat2 patch, put the camera INSIDE it at y≈75 pitch 12, shoot all 4 yaws |
| `SET_ENVPROBE_BRIGHTNESS` | `<float>` | Live scale on `EnvironmentReflection_Global` (engine 1.55 knob `wi::scene::gg_envprobe_brightness`, default 1.0). **0.5 = DX11's dielectric damping** (DX11 ended env reflection with `envColor *= 0.5*metalness+0.5` = ×0.5 on metalness-0 surfaces like sand/terrain), so 1.0 vs 0.5 is the DX12-vs-DX11 reflection-energy A/B for the wet-sand parity question (#155) |
| `SET_PROBEMARKERBRIGHTNESS` | `<float>` | (2.90) The **PER-PROBE** brightness — the `%probe` marker panel's "Probe Brightness" slider, applied to every probe marker in the level, then `g_bLightProbeScaleChanged` exactly as the panel raises it. Travels `eleprof.light.fProbeBrightness` → `g_envProbeList[].brightness` → `EnvironmentProbeComponent::filterBrightness`, and is **baked into the cube** during BRDF mip filtering (engine 2.90, restoring DX11) — so it needs a re-capture, which `SetBrightness` forces by self-dirtying on change. Do not confuse with `SET_ENVPROBE_BRIGHTNESS` above, which is the GLOBAL shader-side knob and needs no re-bake. ⚠ Covers mips 1..N-1: mip 0 is an unfiltered copy, so a roughness-0 mirror surface will not respond (DX11 gap too). Panel range 0.01–10, 1.0 = stock |
| `DUMP_TRANSPARENTS` | `[name-substr]` | Per-subset render state for everything the transparent pass touches (or that is double-sided) → `Files/transparents_dump.txt`. Reports the three fields that decide the hair/leaf depth-write rule — `dsided` (mesh OR material), `blend` (the EFFECTIVE `GetBlendMode()`, not `userBlendMode` — a material can be pulled transparent by its filter mask alone) and `filt&TRANSP` — plus `alphaRef`/`alphatest`, opacity, shaderType and the basecolor name. Rows that the rule applies to are marked inline. Summary line carries the live knob state and the cumulative parity-path draw count. **This is the instrument that named the hair bug**: `adult male hair 10_color.dds` came back `blend=1 dsided=1 alphaRef=1.000 alphatest=0` — alpha test OFF means even alpha≈0 card texels rasterise, so with depth write on they occlude every strand behind them |
| `SET_HAIRDEPTH` | `<0\|1>` | 1 (default, 2026-08-07) = DX11 parity: double-sided TRANSPARENT geometry draws with depth WRITE off, back faces then front faces. 0 = upstream: depth write on, back faces then both faces. Fully live — both pipeline permutations are built at LoadShaders, so no reload is needed (contrast `lazypso`, which must be set before the engine starts). Persistent revert is `setup.ini hairnodepthwrite=0`. **Only alpha-BLENDED materials are affected**: alpha-tested opaque foliage returns `FILTER_OPAQUE`, and GG glass/water take the `customShaderID` branch, so neither reaches this path |
| `SET_WEAPONDEPTH` | `<0\|1>` | 1 (default, 2026-08-07) = DX11 parity for the FIRST-PERSON WEAPON (the fork's `BLENDMODE_FORCEDEPTH`), as **three draws across two passes**: the Z-prepass stamps back faces with compare ALWAYS + depth write (carving the weapon's volume out of the world's depth), then the main pass draws a back-face colour fill (depth read, no write) and the visible front faces. 0 = the DX12 port's behaviour, an ordinary depth test — walk into a crate and it cuts the front half off the pistol. ⚠ **The stamp MUST stay in the prepass and the fill MUST be depth-tested.** Both were tried per-subset in the main pass and both failed the same way: the gun's `ALWAYS` write (depth, then colour) beat the glove drawn as a separate FORCEDEPTH mesh, and the grip rendered over the hand holding it. DX11 only got away with `ALWAYS` because its two halves were two full loops — every mesh stamped before any drew. Applies to every material the DBP `DisableObjectZDepth` command flags (gun + secondary + LegacyArms, HUD layers, importer collision shapes, editor grid plane) — the same set DX11 gave FORCEDEPTH. Persistent revert `setup.ini weaponforcedepth=0` is a TRUE revert (read at material setup too); **`SET_WEAPONDEPTH 0` flips only the renderer half, so it is an A/B lever, NOT the pre-2.09 baseline**. `DUMP_TRANSPARENTS` reports `forcedepth=` per material and `forcedepth_draws=` cumulatively — zero draws with a weapon on screen means the `GG_FORCEDEPTH` flag never reached the renderer, i.e. a game-side fault, not a pipeline one. ★ **Measuring this needs a same-knob control**: the weapon idle animation puts ~0.9% of pixels over an 8-luma threshold between any two frames, which is larger than the effect at a pose where nothing occludes the gun — an OFF/ON diff alone read as a change twice and was wrong both times |
| `SET_WEAPONSHADOW` | `<0\|1>` | GGMAX 2.14 — the SECOND half of the DX11 weapon look (`SHADERTYPE_WEAPON` / `WEAPON_SHADOW`), paired with `SET_WEAPONDEPTH` above. 1 (default) = DX11 parity: inside `light_directional`/`light_point`/`light_spot` the shading position is pulled to **1/3 of the camera distance** before the shadow lookup, so a gun clipping into a wall is not shadowed BY that wall (DX11's own comment: "ensure geometry not STUCK INSIDE a wall that would cast a shadow on it"). 0 = the DX12 port's behaviour — the gun goes dark whenever the player presses against geometry. ★ Unlike `SET_WEAPONDEPTH` this IS a true revert as well as an A/B: the material bit only marks "this is a weapon" and the knob drives a per-frame FrameCB bit the shader tests alongside it, so 0 restores the pre-2.14 look within a frame. Persistent revert `setup.ini weaponshadow=0`. Applies to the SAME object set as FORCEDEPTH and that is faithful, not a shortcut — DX11 set `BLENDMODE_FORCEDEPTH` and `SHADERTYPE_WEAPON` on adjacent lines (DX11 `wickedcalls.cpp:3336-3337`), so the existing `GG_FORCEDEPTH` flag is the correct selector. ⚠ **The harness cannot drive the player**, so this cannot be self-verified: flip the knob and ask for one walk into a wall, which yields both arms at an identical pose (see the 2.09 notes). ⚠ Same measurement trap as `SET_WEAPONDEPTH` — weapon idle motion swamps the effect, always take a same-knob control. Costs nothing: a stash-and-rebuild A/B on Island Showdown read 71.3/71.7 baseline vs 71.5/71.4 with the change |
| `SET_LIGHTFALLOFF` | `<0\|1>` | 1 (default, 2026-08-07) = DX11 light power parity: point/spot lights shade as energy 30 x `(1-d2/r2)^2` with NO inverse-square term and the DX11 linear-to-center spot cone ramp — the broad even flood of the DX11 baseline, reaching the authored range and hitting exactly zero there. 0 = the DX12 port's old look: upstream windowed 1/d2 plus the range2*PI/4 intensity heuristic — a hot pool at the source, mid-range ~half of DX11. FULLY live in ONE knob: the shader branches on a per-frame FrameCB bit (`OPTION_BIT_GG_DX11_LIGHT_FALLOFF`) AND lighting_loop re-pushes every light's intensity through `WickedCall_UpdateLight` each frame reading the same bool, so both halves flip together within a frame — no reload. Persistent revert `setup.ini lightfalloff=0`. Affects ALL punctual lights everywhere (surface, volumetrics, lightmap, DDGI — the branch lives inside `attenuation_pointlight`/`attenuation_spotlight`); the sun/directional path is untouched. `LIST_LIGHTS` shows the live intensity — expect a flat 30 on every GG map light with the knob ON (vs 600-60000 range-derived values OFF) |
| `SET_XINPUT` | `<frames>` | GGMAX 2.15 — gamepad poll throttle, **restores a DX11-fork optimisation the DX12 port dropped**. `XInputGetState` on an EMPTY slot is a driver round-trip, and stock upstream Wicked polls all four slots every frame: on a machine with no pad attached that measured **`Input: 0.29 ms` of a 4.62 ms editor CPU frame (6.3%)** for zero input. 60 (default) = a known-empty slot is re-probed every 60 frames, **staggered so at most ONE empty slot is probed per frame**; 0 = stock every-frame polling of all four, for A/B. **CONNECTED pads are polled every frame at any setting**, so this can never add controller latency — the only behaviour that changes is that a hotplug takes up to `<frames>` frames to notice. The DX11 fork has the identical fix at `WickedRepo/WickedEngine/wiXInput.cpp:18` — `//PE: XInputGetState slow ,so delay if not connected.` with a 120-frame retry — so this is parity restoration, not a new trade. ★ Verify with the new `Input-XInput` / `Input-RawInput` / `Input-SDL` profiler sub-ranges, NOT with FPS: on Switch Escape the range collapsed 0.29 → 0.00 ms and **FPS did not move at all** (201.1 vs 201.2, three arms) because Input sits off that level's critical path. Removing real work and gaining nothing is the expected result there — see `SWITCHESCAPE_PERF.md` |
| `SET_SUBMITSTATS` | `1` | GGMAX 2.16 — reset the rolling submit-stall window, so the next `GET_PERF_DATA`'s new **`SUBMIT_STALL_WINDOW: mean= max= stalled=N/M frames`** line covers only the arm being measured. **Call it at the top of every A/B arm.** Exists because `SUBMIT_PHASES_MS` is a LAST-FRAME SNAPSHOT and `stall` is variable: on Switch Escape it read 0.00 in three captures and 0.38 in a fourth, and the 0.00 was used to wrongly exonerate the GPU fence as the thing absorbing CPU slack. ★ **Judge the stall from the WINDOW line, never from the snapshot.** The window also reports what fraction of frames stalled measurably (>0.05 ms), which distinguishes "steadily fence-bound" from "occasional hitch" — two very different diagnoses that a mean alone hides |
| `SET_SCENESERIAL` | `<0\|1>` | GGMAX 2.15 **DIAGNOSTIC ONLY — never ship enabled.** Names the pole inside `Scene::Update`, which no ordinary profiler range can do: every `RunXUpdateSystem(ctx)` only DISPATCHES jobs and returns, so the entire cost lands in the single `jobsystem::Wait(ctx)` that closes each stage — which is why `Scene-S1 0.95 ms` shows only 0.03 ms of named children. With this on, each instrumented system gets its own `Wait` + `SU-<name>` range (`SU-Animation`, `SU-Physics`, `SU-Transform`, `SU-Hierarchy`, `SU-Mesh`, `SU-Material`, `SU-Object`). ⚠ **Serialising removes cross-system overlap, so the TOTAL inflates** (Switch Escape CPU frame 4.56 → 4.84) — compare the `SU-*` shares against EACH OTHER, never the total against a normal frame. First use (Switch Escape, 2026-08-09) named **`SU-Hierarchy` 0.81 ms** as the biggest single system, with `SU-Object` 0.57 and `SU-Mesh` 0.29 — and **refuted the obvious guess**, `SU-Transform`, at 0.07 ms despite 8437 transforms |
| `SET_INSTINIT` | `0` or `1` | GGMAX 2.18 — A/B the per-frame "blank every instance slot" pass inside `Scene::Update` (`wiScene.cpp:332-340`). `0` = stock Wicked, ONE worker memcpying 256 B x instanceArraySize into write-combined UPLOAD memory every frame (~1.87 MB on Switch Escape); `1` = the identical writes Dispatch'd across the worker pool. Behaviour-neutral by construction (same bytes, same slots), so judge it purely on `Scene-S1`. **MEASURED AND IT IS A WASH: +0.015 ms (+0.9%), inside noise** — it is an `Execute` running alongside the parallel Animation/Physics/Transform dispatches, so it is not the critical path. Kept as an opt-in knob, **default 0**, because it should matter on a scene whose S1 dispatches are cheap relative to instanceArraySize. ⚠ Do not re-raise it as a Switch Escape lever without re-measuring. See SWITCHESCAPE_PERF.md §11.2 |
| `GPUP_DUMP` | (none) | Legacy gpup particle forensics -> `gpup_dump.txt` (settings, cadence counters, per-emitter parsed fields + GPU-visible constants). Result line carries the summary. **Cadence check: two dumps N sec apart — d(time_sum)/emitters/N x 0.015 = sim rate (1.0 = real time); `max_time` must stay <= ~3.2** (worst legal double-accumulation under the 1/30s dt cap) — larger means an uncapped hitch reached the sim, which was the 2026-08-07 "particles reset/balloon" bug (dt cap sat BELOW gpup_update; a lazy-PSO hitch fed it up to 0.5s = a 33-quantum warp in one tick). Since 2026-08-08 also does a **GPU readback of every gpup texture** (`tex` lines: CRC32 + per-channel means + 0/255 saturation %): `noiseOrig`/`dist2` are process-lifetime PNG-sourced inputs whose CRCs are run-invariant constants — **any drift = the texture got scribbled** (white-out task #120 evidence); per-emitter `noise`/`pos`/`speed` are the live sim state, where stats (not CRCs) carry the signal. Costs one GPU stall per texture — a forensics command, not a per-frame poll |
| `GPUP_SHOW` | `<0\|1>` | 0 = skip ALL legacy gpup drawing (sim keeps running) — attribution lever: separates "the steam causes X" from everything else in one flip. Proved the look-up washout pose gets ~1 luma from the 2.10 light curve and up to +16 mean luma from steam pulses |
| `SET_GPUPARM` | `<0\|1\|2>` | Pin the gpup update arm: 0 = default (whole-batch ALWAYS since 2026-08-08 `039f244d`), 1 = whole (same), 2 = force the HERITAGE SPLIT ARM — ⚠ known-broken since the DBPro era (its split==0 loop lacks a break: all emitters tick, lastEmitter runs to 9, split==1 ticks nothing, both frames consume half-quanta → the sim receives HALF of real time). It engaged above 140 FPS, which DX11 never reached but DX12 sits on at steam poses — camera distance flipped the steam between full/half-speed regimes = the user's provokable shape shift. Kept only for A/B archaeology |
| `GPUP_CANARY` | `<0..7>` | Shader debug modes riding the spare padding1/filler constants — no struct change; new effect loads reset to off. **Bypass modes (task #120)**: 1 = every ALIVE gpup particle renders as a fixed-screen-size RED dot at its pool position (bypasses size/alpha/color/gradient sampling + all billboard math), 2 = grid positions (bypasses the pool samples too). THE instrument that proved the white-out veil was NOT gpup pixels — and later (task #122 round 5e) that the "ping" is NOT pool data: canary-1 dots ran 60 s with ZERO pings while the full path pinged 38× the same minute, which also exonerates CB delivery (padding1/filler live IN the CBs), PSO/blend, the t0/t5 pool descriptors and sampler s1 in one stroke. **Subtractive bisect modes (task #122 round 6) — full render minus ONE ingredient**: 3 = no sprite rotation (the sn-driven spin), 4 = no gradient influence (flat color/alpha 0.2), 5 = fixed size gradient, 6 = no distortion offset (texDist2/agk_time scroll), 7 = base image quadrant only (kills the per-slot image selector). Run 60 s per mode with `GPUP_SOLO` — the mode whose ping count collapses names the guilty ingredient |
| `GPUP_SOLO` | `<-1\|n>` | Draw ONLY gpup emitter n (-1 = all; sim keeps running for every emitter either way). Attribution lever for multi-emitter scenes: tracker16 (task #122) ran 40 s captures per mode and named the ping EMITTER 2's DRAW ALONE (ALL=13, SOLO-e2=23, SOLO-e8=0 pings) |
| `GPUP_TRACK` | `<enr>` | Per-particle journey sampler: GPU-readback of texPos/texSpeed[currImage], decodes 120 fixed slots (i=(si*7)%P, j=(si*11)%P) and APPENDS to `gpup_track.csv` (game CWD = Max/Files): S-row (t, enr, curr, sn, rotsn, time, zaehler, spawnint, testpos) + 120 P-rows (t, enr, si, age, x, y, z, sx, sy, sz). Poll it in a loop (~3 Hz max, one GPU stall per call) and analyse continuity per slot. ⚠ Analyse ALL columns: positions/ages read smooth through every ping — the 2026-08-08 catch was in the SPEED columns (20-45 of ~60 live slots kick velocity >4x median in the same 0.5 s window every 1.5-3.5 s), which position-only analysis missed for two rounds |
| `GPUP_DRAWLOG` | `<0\|1>` | Per-FRAME draw flight recorder for emitter 2: every rendered frame appends `AGKTimer, frameCount, currImage, sn, time, gtimer, spawnpos.xy, warp, agk_time, rota.xyz, blendmode, opacity` to `gpup_drawlog.csv` (game CWD). Align against a concurrent ~30 fps window capture to test whether ANY CPU-side per-frame input steps at a visual event. Task #122: 9708 rows proved every logged field frozen/continuous at all 20 pings (and killed the warp-spike theory: spikes at chance level vs pings) |
| `GPUP_AGES` | `<enr>` | FULL age-plane snapshot: decodes ALL P*P slot ages (0-255, slot index = the shader's `fr` = j*P+i) into one row of `gpup_ages.csv` (game CWD). Polled at ~2 Hz, consecutive rows yield per-event BIRTH MASKS. ★ THE instrument that closed #122: masks showed contiguous foreign-window-shaped rejuvenation blocks (up to 3014 births/0.5 s vs the 227/s cursor trickle) — the signature of OTHER emitters' spawn windows leaking into this pool via the shared-CB copy race. Verification on the fixed build = extras ~0, trickle tracking the cursor |
| `GPUP_REBIND` | `<0\|1>` | Decoy-bind lever: 1 = before each gpup draw, bind decoy objects to SRV slots 0-5 + samplers 0-2 so the engine's BindResource/BindSampler compare-skip can never leave a slot un-dirtied — every draw then flushes a freshly copied descriptor table. Built as the conviction test for the stale-descriptor-table theory of #122; result was **A/B-null** (47 vs 44 pings/60 s), which helped EXONERATE the binder and redirect the hunt to the sim. Keep for future descriptor-staleness suspicions |
| `DUMP_SUN` | (none) | Light-shaft chain + sun-flare forensics (engine `GG_DumpSunChain`): shafts enable/fade/strength/sunDot, per-visible-flare-light facing + projected pos (exactly as DrawLensFlares computes), readback stats of rtSun[0] (DrawSun mask) / rtSun[2] (downsample) / rtSun[1] (blur out). Result-line only, ~4 GPU stalls |
| `SET_LENSFLARE` | `<0\|1>` | Toggle wicked lens flares live (`setLensFlareEnabled`). ⚠ In the white-out hunt this "cured" the veil — but so did SET_LIGHTSHAFTS: BOTH were frame-structure confounds (any structural change re-established the flare's GPU state). Only the lensFlarePS red-tint discriminated. Keep as a quick attribution lever, but remember the confound class |
| `GPUP_REGEN` | (none) | Re-create EVERY process-lifetime gpup GPU resource from its in-exe/PNG source on the LIVE instance (safe anywhere; deferred-destroy covers in-flight frames): `texNoiseOrig`/`texDist2` + the five static draw buffers (grid VB/IB pairs + sim quad VB). The white-out (#120) confirm/refute lever — **on a caught white-out, REGEN clearing the screen confirms static-resource poisoning, and GPUP_DUMP's byte-compare says WHICH resource**; no effect on a healthy scene. (Round-1 catch already proved the textures innocent: CRCs unchanged inside a live white-out, and the sim pools were healthy — the buffers are the round-2 suspects) |
| `GPUP_SET_SN` | `<value>` | Fast-forward the gpup `sn` clock (the GPU hash-seed source, grows ~10.4/s forever). THE instrument that proved the 2026-08-08 steam white-out: the shader hash frac(sin(dot(seed,~285x primes))x75633) collapses to a handful of outputs once the dot passes a few million (fp32 ULP 0.5 at 8M) — spawn velocities degenerate into coherent jets and the cloud congeals over ~50 min of runtime. Post-fix (hash reads fmod(sn,256)) the look must be INVARIANT to this value — verified normal at sn=500,000 (=13 h of runtime; pre-fix, 32k was a full white-out). A slow-divergence bug needs a fast-forward lever — build one instead of waiting an hour per repro |
...
| `DUMP_POSTFX` | (none) | The five level-authored post-processing flags in **both** stores at once, plus the Test Game quality preference and the tree LOD distance: `visuals[ssr refl fxaa shafts flare bloom vsync] gamevisuals[...] testgamequality=N treelod=N state=S`. GGMAX 3.87. Built because the defect it verifies is invisible to every other instrument — the editor and the test game each render their own panel correctly, and only a side-by-side of `t.visuals` against `t.gamevisuals`, taken in BOTH states, shows one being stamped over. ⚠⚠ **`testgamequality` is load-bearing in the output**: the stamp is gated on `!= 2` and the shipped default IS 2, so on a box that never lowered *Graphics Quality (Test Game)* the bug cannot occur and a verification run sees no change before OR after. Check that field before concluding anything. ★ `treelod` rides along because it is the SAME preset call and it is still NOT restored on return to the editor (notes §3.86) — expect `3000 → 1000 → 1000` across editor/game/back until that is fixed |
| `SET_REFLECTIONBLUR` | `<0..3>` | Water Reflection Blur - passes of the engine's 9-tap separable Gaussian over the resolved planar reflection target. 0 = off and byte-identical. Writes BOTH `t.visuals` and `t.gamevisuals`; applied by the push in `MasterRenderer::Update`, so it takes one frame - confirm with the `REFLBLUR` line of `DUMP_REFLECTION`. ★ This verb exists because the FIRST 3.90 gate had no way to set the blur and so never executed it, while reporting a pass. A knob with no way to exercise it is a knob with no way to test it. ⚠ At >0 it softens EVERY mirror surface in the level, not only water - there is one shared reflection target. GGMAX 3.90 |
| `SET_REFLECTIONSIZE` | `<0|128..2048>` | Water Reflection Size - the WIDTH of the planar reflection render target; the height follows the main camera's aspect. **0 = Auto**, which is the engine's stock internal-resolution/4 (384x200 on a 1536x801 window) and is bit-for-bit the pre-3.89 behaviour. Writes BOTH `t.visuals` and `t.gamevisuals` (the .fpm saves from gamevisuals) and takes **one frame** - the apply point is the push block in `MasterRenderer::Update`, so read back with `DUMP_REFLECTION` rather than trusting the reply. GGMAX 3.89 |
| `DUMP_REFLECTION` -> `REFLSIZE` | (none) | New line in 3.89: `REFLSIZE visuals=N gamevisuals=N engine=N (0 = Auto = internal/4) rt=WxH samples=4 valid=1`. The three N must agree; `visuals` differing from `engine` means the one-frame push has not run yet or reflections are off. `rt=` is the real target and is the number that proves the setting landed. ⚠ `samples=4` - the reflection is 4x MSAA and that is hardcoded, so the memory cost of the big stops is ~54 bytes per pixel across four targets, not 4 |
| `DUMP_TREEPOOL` | (none) | Tree-pool census + ORPHAN detector (renderable unnamed parent-less objects the tree system no longer owns) to `Files/treepool_dump.txt`. **GGMAX 2.19 changed the first line to `POOL built=N ceiling=M ...`**: `built` is how many pool slots actually exist as ECS entities (the number that costs per-frame Scene::Update work), `ceiling` is `g_treePoolSize`, the most it may grow to. ★ **`built=0` on a level with no trees is CORRECT, not a broken pool** — that is the whole point of lazy growth. Before 2.19 the pool was created eagerly at app startup, so a treeless level still carried 6000 objects + 6000 transforms; Switch Escape's `SCENE_OBJECTS` fell 7322 → 1322 when this shipped |
...
| `DUMP_INSTANCE` | `<name-substr>` | ★ **Its `center`/`radius` are the fastest way to spot a corrupt bounding volume**: a healthy prop reads `center == pos` with a radius of 8-30, while every PICKUP on spotshadowtest read `center=(49650,50021,50077) r=86750` — the midpoint of the object and the library master parked at (100000,100000,100000), because a shared armature’s WORLD-space box is merged into skinned bounds (wiScene.cpp:5231-5241). That 86,730-unit centre distance is what overflowed the fp16 batch distance in 2.32. Read it back for BOTH matches: the parked master shows up as a second row at 100000. Read back the GPU-consumed `ShaderMeshInstance` record for matching renderable objects (uid, color/emissive packs, geometryOffset vs the mesh's, center/radius, transformRaw vs component world) to `Files/instance_dump.txt` |
...
| `DUMP_MESHES` | `[filter]` | **Mesh census** (GGMAX 2.26) → `Files\mesh_census.txt`: every live `MeshComponent` grouped **by GEOMETRY SIGNATURE** (verts/tris/subsets — generator families cluster exactly, which is essential because GG names nearly every loaded mesh `node_mesh`), **by OWNER** (the referencing object's name = the real identity), plus an **ORPHAN** list (meshes no `ObjectComponent` references — a leak detector). With a `filter` it also itemises that one family: entity, owner, parent, **renderable flag**, verts/tris, world position. ★ Built because `SWITCHESCAPE_PERF.md` §16 attributed the entity floor by SUBTRACTION and was left with "75 unexplained meshes" it could not name — enumeration named it in one run: **100 hidden `renderable=NO` decal-pool quads at (0,0,0)** from `decal_init()` (`M-Decal.cpp:41`), see §23. Measured 2026-08-12: ORPHANS=0 on Switch Escape / Trapped / Zombie Cellar (**no mesh leak**), 625 terrain chunks carrying 96% of all triangles, and total mesh count nearly identical across three demos whose POLYS differ 10x. ⚠ Rides `AutoHarness_StandaloneCommands` — a new link in the main dispatch chain re-hits MSVC C1061 |
| `SET_SHADOWEXTRUDE` | `<units>` | **GGMAX 2.28 — the "shadow pops away when its caster leaves the camera frustum" fix.** Directional shadow-CASTER culling extrusion, in world units (inches). Stock Wicked's floor is `min(2000, farPlane) * 0.5` = **1000 units = 25.4 m**, a metres-scale constant in an inch-scale world; a caster displaced further than that ALONG THE LIGHT AXIS was culled and its shadow vanished. In light-view space Z *is* the light axis and a caster shares X/Y with its own shadow, so this extrusion is the only thing that can reject a caster whose shadow is still on screen. Default now 4000 u (~101 m); **`0` restores stock exactly** = instant revert. Fully live (`CreateDirLightShadowCams` reads it every frame), so it is the A/B lever AND the test: stand where a shadow pops, raise it, see if the shadow returns. Persistent: `setup.ini shadowextrude=<N>`. ⚠ Widening the cull is sanctioned, not a hack — depth clip is off for shadows so out-of-projection casters clamp correctly. ⚠ Only lifts the NEAR cascades; cascades 2+ already self-extend via their radius |
| (in `GET_PERF_DATA`) `SHADOW_CASTERS:` | — | `<n> extrude=<u>` — objects the DIRECTIONAL cascade cull selected as casters this frame. ★ **The executed-check for `SET_SHADOWEXTRUDE`**: `VISIBLE_OBJECTS` is blind to it (camera visibility ≠ caster selection), so without this an A/B cannot tell "no cost" from "the knob did nothing". ⚠⚠ **The count is BIMODAL because of the staggered cascade refresh** — Island Showdown reads 43 on most frames and **856** when all cascades refresh, and every high-ms shadow sample is an 856 frame. A naive 5-sample mean measures which refresh phase you caught: on 2026-08-12 that produced stock 1.478 / new 1.124 / **stock-again 0.856**. **Bucket samples on this counter before averaging any shadow-pass timing** |
| (setup.ini) `decalprewarm` | `<N>` | GGMAX 2.27 — decal element pool slots built at startup (default 24 of 100); the rest grow on demand, capped 2/frame. `99` restores the pre-2.27 eager pool exactly and is the control arm for `tools/sceneupdate/decalhitch.sh`. Readback `DECALPOOL: built/prewarm/max/deferred` |
| `DECAL_BURST` | `<n>` | GGMAX 2.27 — place n decals through the REAL allocation path (`decalelement_create`) so the pool's growth can be hitch-tested; the harness cannot fire a weapon. Pair with `HITCH_RESET` then read `HITCH:` — judge the histogram TAIL, never mean FPS |
| (setup.ini) `masterpark` | `<units>` | GGMAX 2.33 — how far out hidden ENTITY MASTERS are parked (`M-Entity_part0.cpp`, two sites). Range 2000..100000, **default 100000 = unchanged**. A placed clone SHARES the master’s MeshComponent, and the engine merges that mesh’s armature WORLD box into every skinned instance (`wiScene.cpp:5231-5241`), so each pickup’s AABB becomes the union of itself and the park position — that is what overflowed the fp16 batch distance in 2.32. Measured A/B on spotshadowtest: default r=86750, `masterpark=4000` r=3625.8, and the OFF path is byte-identical. ⚠ It BOUNDS the error, it does NOT fix it (centre still wrong); the correct fix is engine-side. ⚠ Not defaulted: masters are hidden but present, so moving them inside the play area can expose template geometry to ray picks/physics — needs a 19-demo sweep first. Must be read in the EARLY setup.ini pass (masters park during entity-profile load) |
| `WHYNOTDRAWN` | `<name-substr>` | **GGMAX 2.30 — closes backlog #125 ("why did object X not draw?").** Walks every gate the main camera applies, in engine order, and names the FIRST that rejects: renderable / layerMask / frustum / apparent-size cull (delta 1.30) / occlusion query / presence in the prior frame's `visibleObjects` / draw-distance reject (`wiRenderer.cpp:8663`) / distance-fade dither skip (`:4170`). Also prints the object AABB, centre, radius, `fadeDistance`, camera distance, and whether the camera is inside the AABB (which force-marks it visible at `:4533`). Writes `Files/whynotdrawn.txt`. ★ Built for the invisible collectable pistol, where it killed TWO of my own proposed mechanisms (occlusion, then dither) within a minute each — that is the point of it: a verdict line stops a plausible story being told about a measurement. ⚠ `inVisibleSet` is the PRIOR frame (the harness runs ahead of `UpdateVisibility`); everything else is live |
| `DUMP_BIGAABB` | `[inflationRatio]` (default 2.0) | **GGMAX 2.34 — how much of the level carries an inflated AABB?** `WHYNOTDRAWN` answers "why is THIS object wrong"; this answers "how much is wrong", which is what decides whether an engine-side fix is worth the risk. For every object it recomputes the CLEAN box (`mesh.aabb.transform(world)`, exactly as `RunObjectUpdateSystem` does) and reports those whose real AABB exceeds it by the gate. Prints counts, distinct meshes/armatures blamed, how many exceed the fp16 ceiling (65504), how many are TRANSPARENT and how many are **both** (the only combination that can mis-sort), plus scene bounds and a per-object table with bone count and filter mask. Writes `Files/dumpbigaabb.txt`. ★ **The gate is a RATIO, never a coordinate threshold** — a coordinate test would report "0 affected" the moment `masterpark` moved the park point, and that zero would read as a fix. The ratio also surfaces LEGITIMATE ~2x armature merges (hair, heads), which is how the transparent-sort risk got discriminated instead of assumed. Measured: 2/7249 RPG Template, 5/1167 Zombie Cellar, 17/8558 Island Showdown, 0/1030 Switch Escape — all truly-corrupt entries are 1-bone weapon/ammo pickups; characters are clean. Driver: `tools/sceneupdate/bigaabb.sh <demo>...` |
| `MOVE_ENTITY` | `<name-substr> <dx> <dy> <dz>` | **GGMAX 2.35 — reproduce pickup collection without playing the level.** `SetEntityCollectedEx` (`DarkLUA_part0.cpp:1452`) does NOT hide a collected entity: it teleports it by -999999 on every axis and repositions the object. So `MOVE_ENTITY ammo -999999 -999999 -999999` is the collect path's exact effect on the scene, from the editor, in one command. Pair with `SET_ENTITY_VIS` (which hides instead of moving) to separate a MOVED caster from a HIDDEN one — GGMAX 2.07d's dynamic-caster shadow invalidation catches the first and skips the second at its `if (!object.IsRenderable()) continue;` guard. ⚠ **Always follow with an executed-check** (`DUMP_BIGAABB`): the first use of this compared two screenshots that were indistinguishable, and only the census could tell "the move did nothing" from "the move worked but the object is 4 px wide at this camera distance" (it worked — worst radius 86,926 -> 952,773). Driver: `tools/sceneupdate/pickupshadow.sh <demo> <name-substr>` |
| `INVALIDATE_LOCALSHADOWS` | (none) | **GGMAX 2.35 — force the CACHED local (point/spot) shadow atlas to re-render.** Executed-check for the 2.35 fix: remove a caster, and if the stale shadow vanishes the instant this is issued, the cached atlas was provably what held it. Calls the same engine entry (`wi::renderer::InvalidateLocalShadows`, `wiRenderer.h:1200`) the fix calls from the delete/collect paths, so the experiment tests the shipped mechanism, not a lookalike. Verified on TESTPRO2: ammo hidden -> shadow stays; nudge -> shadow goes; the pistol's shadow survives all three shots as the control. Driver: `tools/sceneupdate/shadownudge.sh <project> <name-substr>` |
| `FIRE_WEAPON` | `[frames]` (default 6) | **GGMAX 2.41/2.42c — pull the trigger as a left mouse press would.** ★ **The trigger is `t.gunmode = 101`, NOT `firingmode`.** `firingmode` is only an ANIMATION input (`G-Gun_part1.cpp:454` gates the run animation with it); the original devs documented this in 2015 at `DarkLUA_part5.cpp:1599` — *"seems when in game, this gets ignored so no gunshoot happens.."* — and their own Lua `FirePlayerWeapon(1)` sets gunmode instead. Only `firingmode>=2` (zoom) writes the field. Test game only. Verified: ammo 29→28. Two code-reading guesses failed before `DUMP_FIRE` settled it. Driver: `tools/sceneupdate/explosionburst.sh` |
| `DUMP_FIRE` | (none) | **GGMAX 2.42 — why did the gun not fire?** Per-frame tracer sampled inside `gun_manager` AFTER every gunclick-zeroing gate: firingmode, gunclick, gunmode, pressedtrigger, mustrelease, ammo, and each gate's inputs, plus `earlyOut` (frames the gun update returned before reading the trigger) and `holdLeft`. Prints a VERDICT naming the stage — the branches are different faults that look identical from outside (early-out / value never arrived / gate zeroed it / edge latch / ammo unchanged / shot fired). Writes `Files/firetrace.txt`. Armed by `FIRE_WEAPON` |
| `BURST_FRAMES` | `<n>` | Capture N CONSECUTIVE rendered frames — the only way to see a sub-second event, since a harness `SCREENSHOT` round-trip costs ~1 s. ⚠ **Three traps.** (1) It writes relative to the GAME'S CWD, which is `Max/Files`, so frames land in `Max/Files/**Files/**screenshots/`, not `Max/Files/screenshots/`. (2) The on-screen FPS counter during a burst measures **PNG encoding**, not the scene — it sinks to single digits and means nothing; never quote a burst frame's FPS as a cost. (3) Frames are **not time-aligned to the event across runs**: the trigger→capture delay varies, so frame N of run A and frame N of run B are different phases. A cross-run frame-N diff is **not** a control — compare the arc (onset → peak → decay), or use a same-run A/B. Driver example: `tools/sceneupdate/explosionburst.sh` |
| `ZOOM_FIRE` | `[zoomframes] [fireat]` (default 120 60) | **GGMAX 2.48 — zoom AND fire through the SHIPPED input path**: holds `g_MouseClickControl` bit 2 (RMB, the Lua zoom state machine advances gunzoommode itself) and presses bit 1 (LMB) mid-hold. ⚠ Do NOT substitute the 2.41 firingmode hold — proven not to fire. Test game only. Driver: `tools/sceneupdate/zoomfire.sh` |
| `DUMP_SHOT` | (none) | **GGMAX 2.48 — what did the bullet RAY do?** One row per fired ray sampled inside `entity_hasbulletrayhit`: zoom state, endpoints, hit id, and `blockedBy` (set in `IntersectAllEx`'s out-of-entity-range discard branch — `blockedBy == gunobj` means the weapon mesh swallowed the shot). Prints a verdict; writes `Files/shottrace.txt`; clears after dump. Companion to `DUMP_FIRE`, one stage further down |
| `CLICK_NODE` | `<title>` | Extended in 2.49: a LEVEL node with an EMPTY level file now opens the **Terrain Generator** (mirrors the storyboard recipe, `M-GridEditB_part19.cpp:2629`) instead of erroring |
| `TERRAINGEN_GENERATE` | (none) | **GGMAX 2.49 — press "Generate Terrain and Open the Level Editor"** (arms the 5-frame countdown; the crash lived in its screenshot block). ⚠ Skips the button's entity re-banking preamble — built for crash-fix verification, not a full button substitute. Requires the generator open |
| `TERRAINGEN_STATE` | (none) | procedural / quitCountdown / saveAsOpen — gate scripts on `saveAsOpen=1` for "Generate survived" |
| `TERRAINGEN_BACK` | (none) | **GGMAX 2.54 — exit the generator to the storyboard** (the back arrow's storyboard route, minus its MODAL askBoxCancel). Built for the enter→exit→re-enter wipe verification: re-entry must show `TERRAIN_RING chunks` collapsing then rebuilding (2.54 Generation_Restart on the `bProceduralLevel` false→true transition) |
| (GET_PERF_DATA) `TERRAIN_RING` extras | — | **GGMAX 2.68h**: the ring line now ends `emptyV/G/E` (bEnableEmptyLevelMode in t.visuals / t.gamevisuals / t.editorvisuals — which COPY survived a level switch) + `hidden` (wickedTerrainHidden). Built for the ssss10 hunt: all four read correct while the "terrain" still rendered = the surface was NOT wicked chunks (it was the resurrected LEGACY draw — see §2.68i notes; terrain-mask PICK_AT missing a visible ground surface is the signature) |
| `TERRAINGEN_BIOME` | `[1-7\|plains\|desert\|forest\|snow\|canyon\|mountain\|rainforest]` | **GGMAX 2.62 — click a biome button / read biome state.** No arg: `sel/ptype/seed/amp/offx/offz/editsize/slopemat0/treebits` + `marker=(x,z)` since 2.65 (the marker rests where the last drag dropped it; the Generate-button fold is `off += M2O(U2M(-marker))` = marker × 0.0254 × 0.0007874) + `markerScr=(x,y)` since 2.68c (the marker's PROJECTED screen position — real-cursor probes must grab THERE +23px on Y, never at hardcoded coords: the screen Y is seed-dependent) + `grass drawEn/wOn/hairs/vis/strands` since 2.67 (legacy flag vs the REAL wicked gate vs hair systems present/unhidden — hairs only build near a LOW camera; drawEn=wOn flipping with vis is the Vegetation-checkbox proof) (sel mirrors the panel's selection static, 1-frame stale). With arg: injects the click through the SHIPPED button branch (M-TerrainNew_part5 iRandomThemeChoice lane) — generator panel must be drawing. Success signature after a click: sel/ptype/seed all change AND `TERRAIN_RING chunks` collapses then refills (the 2.62 params-notify wipe; before 2.62 the ring froze = Lee's dead-Desert repro) |
| `PICK_AT` | `<x> <y> [layerMask]` | **GGMAX 2.64 — scene pick at client coords** (same space as SCREENSHOT pixels): hit/pos/entity + marker-map verdict + the marker object's own frame entities. Bypasses GetPick2's ImGui-focus early-out (harness-time picks are otherwise dead). Proved the generator marker is NOT scene-pickable (ray passes through it) |
| `IMGUI_PROBE` | (none) | **GGMAX 2.65 — what ImGui itself sees**: io.MousePos + button state, hovered/nav window names, display/viewport rects, Save-As modal rect when open. Built when the modal's Cancel ate two real-cursor clicks; named the miss in one shot. ★ Rule for real-cursor probes: ImGui's client area starts 23 px BELOW the screen top (title bar at this rig's 125% DPI; disp=1536x801) — screenshot `(x,y)` → set cursor to `(x, y+23)` for CLICKS. X is 1:1. Tall targets (marker 28 px radius, the 52 px Generate button) masked the error for weeks; short buttons expose it |
| `TERRAINGEN_MATS` | (none) | **GGMAX 2.63c — terrain material forensics**: each wicked slot's resolved matN folder + painted-material-map histogram + chunk(0,0) blendmap layer weights (born flag, per-layer sums). The dead-link namer for any "terrain looks wrong" hunt — proved the stale-biome centre had PERFECT blend data (L3=100% onto the right slot) and the fault was the GPU-side stale ShaderMaterial |
| `SET_TERRAIN_GEN` | `<4..24>` | **GGMAX 2.54a DIAGNOSTIC** — set the terrain chunk ring radius live (default 12 = 625 chunks; 19 = 1521 = 5 km coverage). Grows within ~20 s; ⚠ does NOT shrink below `generation+2+removal_margin(12)` — chunks linger by design until a fresh entry (the 2.54 wipe) resets. Never leave changed: POLYS/VRAM comparisons are invalid while it differs |
| `TERRAIN_GENPROF` | `[RESET]` | **GGMAX 2.58 — per-phase chunk-generation cost breakdown** (avg ms/chunk since launch/reset). Named the 8.2 ms/chunk pick-BVH build (75% of generation). ⚠ renderdata+bvh are async-timed and overlap physics; only `total` is generator-thread wall |
| `MERGE_PROF` | (none) | **GGMAX 2.60 — MergeFastInternal cost per ComponentManager** (cumulative). Exonerated the merge for the generator-fill S1 frames: 423 ms TOTAL per fill, max call 5.1 ms |
| `VT_PROF` | `[RESET]` | **GGMAX 2.61 — main-thread VT/texture-allocation attribution** (cumulative): UpdateVirtualTexturesCPU calls/total/max + entry wait, vtinit/resinit (residency pool misses), main-thread region-texture creates, device-wide blocking copy-queue waits. Refuted the 2.60 "VT allocation wall" claim (updcpu 0.36 s/fill, resinit N=0). ⚠ RESET also zeroes the copywait counters; counters are cumulative — reset BEFORE the window you mean to measure (startup ring pollutes) |
| `VIDEO_TEST` | `<ABSOLUTE path .mp4>` | **GGMAX 2.50 — drive the tutorial-video pipeline headlessly** (LoadAnimation slot 30 → PlayAnimation → WMF decode → YUY2→RGBA → DX12 bridge upload). ⚠ ABSOLUTE path only (MF rejects relative, and the failure path is a MODAL that hangs a headless run). ⚠ ANIMATIONMAX=33: out-of-range slots fail silently and playing a dead slot is modal too. Unbuffered forensics: `Files/videotrace.txt` |
| `VIDEO_STATUS` | (none) | view handle (non-null AND changing between polls = decoded frames reaching GPU textures), percent, playing |
| `HUD_EDIT` / `HUD_ADD_IMAGE` / `HUD_DUMP` | `[title]` / `[path]` / (none) | **GGMAX 2.36 — drive the SCREEN (HUD) editor.** `HUD_EDIT` opens the screen editor on a storyboard node by title (default `In-Game HUD`); `HUD_ADD_IMAGE` adds an image widget centred at 50,50 and points it at a path (default `imagebank\hudmmo-health-panel.png`), mirroring the UI exactly — `AddWidgetToScreen` plus the same `iUpdateWidgetThumbNode/Button` reload trigger the Select Image control uses; `HUD_DUMP` lists every widget with its path, thumb id, **exist** and pixel size -> `Files/hudwidgets.txt`. ⚠ **`exist` means LOADED, not VISIBLE** — measured on TESTPRO2, the ammo-health panel reports `exist=YES 235x139` and the widget box is even sized from those dimensions, yet the canvas draws nothing. Do not read a passing `exist` as "the image is on screen"; it narrows the fault to the blit. Driver: `tools/sceneupdate/hudimage.sh [project] [imagepath]` |
| `SET_OCCLUSION` | `0` / `1` | GGMAX 2.30 — GPU occlusion culling on/off, live. ⚠ Writes BOTH `wi::renderer::SetOcclusionCullingEnabled` AND `t.visuals.bOcclusionCulling`, and must: the Graphics panel reconciles the engine to the visuals value whenever it is open (`M-GridEditB_part24.cpp:134`) and the visuals-apply path pushes it again (`M-GridEditB_part3.cpp:1815`), so an engine-only write would be silently reverted. The reply prints both so a divergence is obvious. ★ Occlusion culling is a per-level SAVED setting — an intermittency source in its own right |
| `DUMP_RAGDOLL` | `[reset]` | **GGMAX 2.29 — the stage-namer for ragdoll death (issue D).** Walks the whole chain: `RAGDOLL_CREATE:` (calls / obj / frames / bip prefix / pelvis / spine / existed / produced / limbsTagged) → `RAGDOLL_UPDATE:` (calls / bones / framesMoved / animateFrom / stillMoving / writebackCalls) → `RAGDOLL_WB:` (entered / animRef / animSet / animComp / channel / **applied** / **splitTarget** / knob) → `RAGDOLL_REG:` (the game's own `t.Ragdolls[]` table) → `RAGDOLL_VERDICT:`. Each WB counter is one nesting level deeper than the last, so the first zero IS the answer. ★★ **Judge survival by `clobbered`/`survived`, NOT by the `HELD` on the RAGDOLL_BONE line** — the harness runs in `GuruLoopLogic`, the same phase as the Bullet step, so a dump can read back its own write and print HELD while the animation system clobbers the bone a moment later. That exact false pass happened on 2026-08-12; `clobbered=225 survived=0` is what convicted the looping clip. The clip-stop fix inverted it to `clobbered=0 survived=249`. ★ Built because the 2.28 fix (restoring the writeback CALL) did not work and code-reading had stalled twice — four stages were unverified and the T-pose looked identical whichever one was dead. ⚠ **`splitTarget` must be 0**: the single merged `SetPreFrame` is only valid while a bone's pos and rot channels share a target entity; non-zero invalidates the 2.29 writeback. `reset` zeroes the counters after printing. ⚠ Ragdolls only update in TEST GAME — `ragdollManager::Update()` is driven from the Bullet step, which the editor does not run |
| `RAGDOLL_TEST` | `[entityIndex]` | GGMAX 2.29 — ragdollify a character without a player pulling a trigger (the harness provably cannot drive the player). Reproduces exactly what `G-Entity_part1.cpp:722-732` does when the death path picks ragdoll. No argument = first live entity whose profile has `ischaracter==1` and a `Bip01_Pelvis` limb. Follow with `DUMP_RAGDOLL` and a `SCREENSHOT`. Driver: `tools/sceneupdate/ragdolltest.sh` |
| `SET_RAGDOLLWRITEBACK` | `0` / `1` | GGMAX 2.29 — `1` (default) writes ragdoll bone poses onto the visible mesh via the `GGAnimBridge` preframe path; `0` restores 2.28, where the writeback runs and applies nothing. The A/B for "was the dead writeback the cause". Persistent: `setup.ini ragdollwriteback=0` |
| (setup.ini) `terraingen` | `<N>` | **wi::terrain chunk RING RADIUS**, shipped default 14 — `(2N+1)^2` chunk entities (14 → 29x29 = **841**), each a mesh + material + transform + hierarchy node that `Scene::Update` walks EVERY frame. This is the DX12 entity floor: DX11 has no `wiTerrain` at all and draws terrain with zero ECS entities (`SWITCHESCAPE_PERF.md` §16). One-shot binding like `treepool` — the terrain is created once by `GGTerrainWicked_Init` and nothing re-reads `generation`, so it MUST be set here and a runtime knob would be inert. ⚠ **Lower is not automatically free: the ring is what COVERS the map.** Radius = `N * 5280` units; the playable map is `+/-editable_size` (the Terrain Editable Size slider, 0.5-5.0 km, stored as the HALF-size in inches — 1 unit = 1 inch). Read `TERRAIN_RING` below on the actual level before changing it |
| (in `GET_PERF_DATA`) `TERRAIN_RING:` | — | `gen` / `chunks` (live) / `ringMax` = `(2*gen+1)^2` / `chunkU` = `(chunk_width-1)*chunk_scale` = 5280 / **`viewM` = `gen*chunkU` in metres, the terrain view distance AROUND THE CAMERA** / `centreToCam` / `mapHalfM` (context only). ★ **`gen` is a view distance, NOT map coverage**: GG sets `SetCenterToCamEnabled(true)` and the engine recomputes `center_chunk` from `camera.Eye` every frame (`wiTerrain.cpp:776-780`), so the ring travels with the player and the editable map size is irrelevant — nothing is ever cropped at any map size. ⚠⚠ **Do NOT reintroduce a `needGen = ceil(mapHalf/chunkU)` column.** An earlier version of this line did, and it produced a confident false finding that two 5 km demos were cropped; flying to x=90000 on A Grand Canyon Adventure shows solid ground (`SWITCHESCAPE_PERF.md` §22.7). Measured 2026-08-11: Island Showdown and Switch Escape both `chunks=841=ringMax` at gen 14, i.e. the FULL ring is built with a parked camera (the "settles ~650-700 of the 841" note in `GGTerrainWicked.cpp` is wrong for that case). ⚠ `chunks` can EXCEED `ringMax` after the camera moves — removal lags creation by `gen+2+gg_removal_margin` rings (898 and 1020 both measured) |
