...
| `FIRE_WEAPON` | `[frames]` (default 6) | **GGMAX 2.41 — hold the trigger as a left mouse press would.** Sets `g_ggFireHoldFrames`, consumed at the top of `physics_player_gatherkeycontrols` (`M-Physics_part1.cpp`), the ONLY writer of `t.player[1].state.firingmode`, so the value is applied at the right point in the frame and across frames. Also sets the engine's own script hook `g.playeraction=1`. Test game only. ⚠ **DOES NOT YET PRODUCE A SHOT** — measured on testpro2: ammo stays 30/388 and the barrel survives, with both a 1-frame pull and a 6-frame hold. `firingmode` turns out to feed only the RUN-animation gate (`G-Gun_part1.cpp:454`); the real trigger runs through `t.gunclick` (`G-Gun_part0.cpp:227-231`) and an edge latch `t.gunandmelee.pressedtrigger` / `t.gunmustreleasefirst`. Next step is an instrument printing gunclick/gunmode/pressedtrigger per frame, NOT another guess. Driver: `tools/sceneupdate/explosion.sh` |
