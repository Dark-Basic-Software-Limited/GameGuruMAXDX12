...
| `SET_LOCALPROBES` | `<0\|1>` | (2.78, #157) 0 parks every local pool probe (out of range, 20000 units up) so the whole level — terrain, props and the %probe marker balls — reflects ONLY the GLOBAL/base env cube. The held version of dragging Probe Range to glimpse the base map. ★ **Persistent form: `globalprobeonly=1` in setup.ini** — survives exit/relaunch and level loads (verified cold: pool parks itself with no harness command). ⚠ BOTH tracking paths are gated; gating only the placed-probe branch lets the old 2-swap path re-assign the pool on the next frame |
| `SET_DEBUGPROBEGLOBAL` | `<0\|1>` | (2.78, #157) The picked probe marker's preview sphere shows the **GLOBAL/base env cube** instead of its own local capture, continuously. Replaces Lee's "slide Probe Range and catch it in snatches" repro. Position/size still come from the picked marker; only the source cube changes. The engine picks the global probe by largest range (50000+ vs hundreds for placed probes) |
| `SET_ENVONLY` | `<0|mode> [mip]` | (2.79, #157) Opaque objects output ONLY the env term — materials/lights/AO/GI/fog stripped. Modes: 1 cube via reflection, 2 via facenormal, 3/4/5 N/facenormal/R painted as colour, 6 +ambient, 7 ×fresnel, 8 ×occlusion, 9 ambient alone, 10 sky alone. Persistent: `envonly`/`envonlymip` in setup.ini |
| `SET_ENVSOLID` | `<0..5> [r g b]` | (2.80/2.81, #157) Override every read of the GLOBAL env cube. **1** = flat colour (default magenta) at all 3 read sites; **2** = SPLIT map — magenta=specular, green=ambient, blue=parallax-local (the site map that proved the ball's disc is 100% the specular read); **3** = paint the chosen mip index as colour (red0/green1/blue2/yellow3/white4+); **4** = force mip to `r` and sample; **5** = ★ +X FACE WIPE (Lee-directed) — normal render but any cube sample whose direction is dominated by +X returns BLACK, at all 3 sites; proves live shader access to the cube. Persistent: `envsolid=<mode>` in setup.ini (int passthrough since 2.81 — before that the ini path collapsed every mode to 1, which is what faked the 08-17 "shader edit not live" failure) |
| `SET_ENVDIR` | `<0-4> [x y z]` | (2.82, #157, Lee-directed) DIRECTION-PEEL rungs — remove the contributors that bend the env-cube sample direction, one at a time (cumulative). The verified chain (with the bend each rung removes): mesh vertex normals → backface flip (objectHF:563) → normal map (objectHF:843) → `R = -reflect(V,N)` camera fold-in (surfaceHF:280) → box projection on the local path (lightingHF). **0** stock; **1** box projection OFF (local path samples raw R); **2** + normal map OFF (reflect off `facenormal` — snapshotted AFTER the backface flip, BEFORE the normal map); **3** + camera OFF (sample the geometric normal, no reflect); **4** FIXED direction xyz (default +Z) at every read site — no direction left, env term = one texel = flat wash; **5** ★ CARDINAL LOCK (2.83, Lee's test) — snap the stock direction to its dominant axis: every read hits a face CENTRE dead-on (no diagonals, no edge filtering); if the discs survive this, direction is cleared in every form and only the MIP choice remains. Composes with SET_ENVSOLID (wipe/mip/split operate on the peeled direction). ⚠ Mesh vertex normals + the backface flip cannot be peeled individually at read time (they ARE facenormal); rung 4 removes them with everything else. ⚠ MIP still rides roughness — pair with `SET_ENVSOLID 4 <mip>` to freeze it. Persistent: `envdir=<mode>` in setup.ini (int passthrough; xyz ini-side fixed at +Z) |
| `SET_PROBEMIPS` | `<0\|1>` | (2.85, #157, Lee-directed) **1 = PLAIN MIPS (DEFAULT)**: skip Wicked's BRDF prefilter entirely, ship the ordinary 2x2 box-reduction chain — the DX11-style mips (a mip texel is only ever a local average of its own face patch; roughness plays no part). 0 = BRDF-filtered mips (with the 2.84 fixes). Applies on the NEXT capture — follow with `REFRESH_ENVPROBE` |
| `SET_PROBEFILTER` | `<hdrclamp>` | (2.84, #157 fix 3) Clamp HDR samples inside the env-probe BRDF prefilter (FilterEnvMap). Default 2.0 — kills the 10-20x blown horizon band that flooded the filtered mips flat (the disc-bug "gaps"); **0 = stock Wicked filter** (the A/B revert). Applies on the NEXT capture — follow with `REFRESH_ENVPROBE` to re-bake and see it |
| `SET_GLOBALPROBE` | `<x> <y> <z>` \| `rebake` \| `off` | (2.78, #157) Move the GLOBAL env probe's CAPTURE POINT and re-bake it there. ★ Stock captures at the **MAP CORNER** (x=0, z=0) at the biome's flat height — measured (0, 8215, 0) on Lee's island, i.e. 208 m up over the corner, which is why the base cube's content resembles no part of the level. The original intent is still in GGTerrain_part0 as commented code: sample the ACTUAL ground at the origin and rise 30 m. `off` restores stock, `rebake` re-captures in place. Confirm with `DUMP_ENVPROBE` (prints probe[0] pos) |
| `SET_PROBECAPTURETRACE` | `<0\|1> [radius=400]` | (2.77, #157) Engine appends one block per env-probe CAPTURE to `Files/probecapture.txt`: the capture's znear/zfar **inherited from the MAIN camera** (each cube face clips there, so the hidden volume around a probe is a CUBE of half-width znear — measured 3.0 units) plus every object within radius with its exact keep/skip reason (layer / range sphere / renderable / NOT_VISIBLE_IN_REFLECTIONS). **This is the command that named the circles bug**: it showed the %probe marker's second object coming back `norefl=0 RENDERED` inside its own probe. Turning it on truncates the file. Leave OFF unless tracing — captures can run every frame during a marker drag |
| `DUMP_OBJENT` | `<obj>` or `<start> <end>` | (2.77, #157) Walk a GG object's frames to their wicked ObjectComponents: frame name, entity, component present, NOT_VISIBLE_IN_REFLECTIONS, renderable. Range form writes `Files/objent_dump.txt`. ★ Use the RANGE form on markers: **a %probe marker owns TWO GG objects** (element obj = inner 'sphere', plus a separate 'root' shell) and the element table names only one |
| `DUMP_ENTOBJ` | `<wickedEntityID>` | (2.77, #157) Reverse map a wicked entity to its GG object number; pair with the `ent=` ids in probecapture.txt. ⚠ `-1` is NOT proof the entity is engine-side — the lookup only scans the object short-list |
