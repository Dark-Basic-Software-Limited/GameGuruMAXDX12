...
| `SET_DEBUGPROBES` | `<0\|1>` | (2.74b) Force the engine's debug env-probe mirror spheres on/off without a picked probe marker (lighting_loop otherwise clears the flag every frame; this is a sticky override). The debug sphere is the TRUE data view of a probe cube. ⚠ **The big ball a user sees when selecting a probe marker is the `%probe` ENTITY (probe.dbo, scale 50, selection outline) — NOT this debug sphere**: its per-face "porthole" look is a property of that entity's PBR shading, while the underlying cube map is seamless (proof chain in §2.74b: DUMP_ENVPROBE → panorama reprojection → offline mirror-ball sim → this debug sphere, all clean) |
