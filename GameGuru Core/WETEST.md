...
| `VERIFY_MESH` | `[name-substr]` | **The GPU mesh-buffer content oracle** (empty arg = every mesh). Per mesh: snapshot `generalBuffer` bytes → `CreateRenderData()` re-upload from CPU arrays → snapshot → byte-compare per region (ib/vb_pos_wind/nor/tan/uvs/atl/col/bon). Mismatch = GPU content differed from what CPU data produces = corruption (now healed); report with first-diff offset + A/B hex samples → `Files/verify_mesh.txt`, summary REWRITES `auto_result.txt` on completion (async, seconds). Meshes with freed CPU arrays are SKIPPED (unverifiable). Re-uploads are budgeted (48/frame) — a one-frame storm of hundreds of CreateRenderData is itself the allocator-churn trigger. 30s watchdog aborts cleanly if the device dies mid-run. CAVEAT: ~420 island meshes (skinned + some statics) report 100%-different regions on every run — suspected legitimately non-deterministic rebuilds or verify-wave self-stomping, cross-check with `alloc_tripwire.txt` before trusting; terrain `chunk_` meshes are byte-stable and fully trustworthy |
...
| `SET_TREES draw` | `0` or `1` | (new SET_TREES param) 0 = `ggtrees_draw_enabled=0` + `GGTrees_HideAll()`; 1 = re-enable + force a pool rescan. Elimination probe for tree-pool involvement |
