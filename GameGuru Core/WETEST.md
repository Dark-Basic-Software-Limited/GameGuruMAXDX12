...
| `GPUP_CANARY` | `<0\|1\|2>` | Shader debug mode (task #120): 1 = every ALIVE gpup particle renders as a fixed-screen-size RED dot at its pool position (bypasses size/alpha/color/gradient sampling + all billboard math), 2 = grid positions (bypasses the pool samples too), 0 = off. Rides the spare padding1/filler constants — no struct change; new effect loads reset to off. THE instrument that proved the white-out veil was NOT gpup pixels: forced all-red changed nothing in a live white-out |
