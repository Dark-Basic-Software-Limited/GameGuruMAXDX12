...
| `SET_ENVDIR` | `<0-4> [x y z]` | (2.82, #157, Lee-directed) DIRECTION-PEEL rungs — remove the contributors that bend the env-cube sample direction, one at a time (cumulative). The verified chain (with the bend each rung removes): mesh vertex normals → backface flip (objectHF:563) → normal map (objectHF:843) → `R = -reflect(V,N)` camera fold-in (surfaceHF:280) → box projection on the local path (lightingHF). **0** stock; **1** box projection OFF (local path samples raw R); **2** + normal map OFF (reflect off `facenormal` — snapshotted AFTER the backface flip, BEFORE the normal map); **3** + camera OFF (sample the geometric normal, no reflect); **4** FIXED direction xyz (default +Z) at every read site — no direction left, env term = one texel = flat wash. Composes with SET_ENVSOLID (wipe/mip/split operate on the peeled direction). ⚠ Mesh vertex normals + the backface flip cannot be peeled individually at read time (they ARE facenormal); rung 4 removes them with everything else. ⚠ MIP still rides roughness — pair with `SET_ENVSOLID 4 <mip>` to freeze it. Persistent: `envdir=<mode>` in setup.ini (int passthrough; xyz ini-side fixed at +Z) |
