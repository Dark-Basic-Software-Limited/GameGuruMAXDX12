...
| `SET_GRASSLOD` | `<0\|1> [step2frac step4frac boost]` | A/B Wicked delta 1.49: grass strand LOD. Beyond `step2frac`×viewDistance only every 2nd strand draws, beyond `step4frac`× every 4th (hashed strand id — stable at a parked camera, no temporal pops); survivors widen ×boost per step to hold coverage. GG grass systems only. **Default OFF (visual trade — user decides).** Measured TESTPRO1 gate view: 72.6→79.2 FPS at defaults (0.35/0.60 ×1.8), →80.9 at 0.25/0.45; screenshots indistinguishable at the test camera |
