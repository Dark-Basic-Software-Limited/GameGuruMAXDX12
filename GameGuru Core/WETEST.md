...
| `SET_TANGENTVIS` | `<0-5>` | Engine 1.62 debug: object shading replaced with raw pipeline data. 0=off, 1=world tangent RGB, 2=interpolated vertex normal RGB, 3=final bumped normal RGB, 4=tangent handedness (red=-1 green=+1 — also maps which parts are DBO-tangent vs MikkTSpace), 5=strength-scaled normal-map sample rg. Pair with `BURST_FRAMES` for per-frame stability forensics. Flicker-hunt result 2026-07-30: mode 1 churn 0.19 (tangents STABLE), mode 5 churn 3.14→1.89 when `SET_STREAMING 0` (the flicker rides the SAMPLE) |
