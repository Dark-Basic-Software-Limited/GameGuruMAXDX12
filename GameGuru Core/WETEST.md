...
| `SET_TERRAINTILE` | `<cap>` (0-255) | Wicked delta **1.53b**: terrain VT tiling repeat CAP. Every bake rung finer than `cap` repeats-per-chunk collapses to the cap scale (pure downsample chain = INVISIBLE transitions from the camera through the mid field); rungs at/below the cap keep the stock repeat-halving anti-tiling far away. **Doubles as the terrain texture world feature size: chunk ≈5120in → cap 8 ≈ 16m repeat (bold), 16 ≈ 8m (default), 32 ≈ 4m (busy). 0 = stock (two-scale cross-fades right at the camera).** World-anchored: no chunk-border seams, no promotion scale-pops. Change queues a fast repaint of resident chunks (brief FPS dip, ~seconds); zero steady-state cost. History: v1 (share-mips K) FAILED in the user's field test — it anchored to sub-magnification top rungs and only densified the visible pattern |
