...
| `SET_SINGLEQUEUE` | `0` or `1` | A/B Wicked delta 1.48b: route COMPUTE/COPY command lists onto the GRAPHICS queue + drop same-queue fences (batches 15→1, deps 12→0). ⚠ **THAT VERDICT IS STALE — re-measured 2026-08-09 and it is now the single best FPS lever found: Switch Escape 201→225.3 (+11.9%), Island Showdown 91.6→95.1 (+3.8%), no visual change.** (Was −4.7 FPS on TESTPRO1 on a much older engine; I expected a light-vs-heavy crossover, tested a heavy level for it, and there isn't one.) Why it wins: the editor frame is GPU-FENCE-bound — CPU waits on the fence in 99.4% of frames — and ~1 ms of GPU frame is cross-queue bubble at `deps=9`. **Default still 0** because Island Showdown's *max* stall got worse (3.37→6.71 ms) even as its mean improved; flipping needs the 19-demo sweep gated on the `HITCH` tail (`SWITCHESCAPE_PERF.md` §9). Judge it with `SET_SUBMITSTATS 1` + `SUBMIT_STALL_WINDOW`, not the snapshot below. The 1.48a `SUBMIT_PHASES_MS` readout in GET_PERF_DATA shows close/fences/present/sync/stall + lists/batches/deps — the 'submit tail' is ~95% the end STALL (GPU still busy) = the frame is GPU-WALL-bound |
