...
| `SET_TERRAINTILE` | `<K>` (0-255) | Wicked delta **1.53**: terrain VT tiling share-mips. The K mips below each chunk region's mip0 keep mip0's texture repeat count (pure-downsample = INVISIBLE trilinear transitions near the camera); beyond K the stock repeat-halving anti-tiling resumes for the far field. **0 = stock (two-scale cross-fade bands right at the camera — the "swimming mess underfoot"); default 1; higher = uniform scale carried further out (K=3 ≈ one scale across the whole near view) at the cost of more visible repetition at mid distance.** Change queues a fast repaint of resident chunks (brief FPS dip, ~seconds); zero steady-state cost |
