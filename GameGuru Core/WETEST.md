...
| `GPUP_DUMP` | (none) | Legacy gpup particle forensics -> `gpup_dump.txt` (settings, cadence counters, per-emitter parsed fields + GPU-visible constants). Result line carries the summary. **Cadence check: two dumps N sec apart — d(time_sum)/emitters/N x 0.015 = sim rate (1.0 = real time); `max_time` must stay <= ~3.2** (worst legal double-accumulation under the 1/30s dt cap) — larger means an uncapped hitch reached the sim, which was the 2026-08-07 "particles reset/balloon" bug (dt cap sat BELOW gpup_update; a lazy-PSO hitch fed it up to 0.5s = a 33-quantum warp in one tick) |
