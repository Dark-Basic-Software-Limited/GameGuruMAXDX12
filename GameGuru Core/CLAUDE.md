...
- **Phase 4+ — Grass improvements**: orientation fix landed 2026-06-18 (Wicked-side, see [WICKED_ENGINE_CHANGES.md](WICKED_ENGINE_CHANGES.md)). Per-grass-type DDS textures now wired up — each entry in `GGGrass::grassFiles[]` gets its own `MaterialComponent` + `HairParticleSystem` appearance template; `ProcessGrassChunks` buckets vertices by painted type id and spawns one hair entity per used type. Blades 2× the previous size for DX11 parity; sway is smooth (FP32 position buffer instead of UNORM). Brush cursor (green ring) now visible on Wicked terrain via a `DecalComponent`. **Next**: actually paint into `pGrassMap` on left-click-drag so the cursor places grass. The legacy `GGGrass_Update_Painting(RAY)` function already exists in `GGGrass.cpp:1393` and writes the grass map — needs to be hooked up to the Wicked-side raycast result.
