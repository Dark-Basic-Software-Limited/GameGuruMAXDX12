...
- **Phase 4+ — Grass improvements**: orientation fix landed 2026-06-18 (Wicked-side, see [WICKED_ENGINE_CHANGES.md](WICKED_ENGINE_CHANGES.md)). Per-grass-type DDS textures now wired up — each entry in `GGGrass::grassFiles[]` gets its own `MaterialComponent` + `HairParticleSystem` appearance template; `ProcessGrassChunks` buckets vertices by painted type id and spawns one hair entity per used type. Remaining improvements are small refinements: per-category sizing tweaks, biome-aware fallback for legacy value-1 paint (currently aliased to type 0 = Course Grass mat1), atlas variety/randomness, paint UX. Implementation lives in `Guru-WickedMAX/GGTerrain/GGTerrainWicked.cpp` (`BuildGrassMaterial`, `BuildGrassAppearance`, `ProcessGrassChunks`). The OLD `GGGrass_Draw*` callbacks are effectively dead in runtime (Z key toggles them with no visible effect) but the data layer (`pGrassMap`) is still authoritative.
