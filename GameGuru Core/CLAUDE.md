...
- **Phase 4+ — Grass complete + community customization**: orientation fix (Wicked-side, [WICKED_ENGINE_CHANGES.md](WICKED_ENGINE_CHANGES.md) entries 1.1-1.4), per-grass-type DDS textures, FP32 sway, brush ring cursor, grass paint live. **Stage 3 Option B**: per-strand grass-map sampling in `hairparticle_simulateCS.hlsl` (brush footprint == grass footprint, no ~4× amplification). **Stage B.5** anchors blade lengths to DX11 `grass_scale = 40` and applies `_SF_x.xx` scaleFactor to width per `GGGrassVS.hlsl:45`. **Stage B.6** wires the **Grass Draw Distance** slider into both per-strand viewDistance and the chunk-entity outer ring (1 chunk further out so strands fade in one-by-one instead of whole chunks popping). **Stage B.8** stripped remaining kelp/seaweed/weed/flower width and length hard-codes — all 8 categories now pure DX11 mapping. **Stage B.9 (community feature)** adds the **custom grass palette**: "Add New Grass" / "Delete Grass" buttons on the vegetation panel; 22 stock + up to 42 custom slots (cap set by `paint_type` uint64 bitmask). Custom slots persist to `.fpm` via the existing `sGrassTextures[128]` save/load. Byte encoding: stock uses grassMaterialTypes[mat][slot]+2, custom uses slot+26 direct (real_types disjoint). Sync from `sGrassTextures[]` to `GGGrass_SetCustomSlotFilename` hooked into `Wicked_Update_Visuals` so custom grass renders immediately on level load. **Advanced settings for grass (per-category / per-slot length + width + density tuning UI) is the next community-facing track.** Otherwise: **Phase 5** — trees / colored cylinder placeholders driven from `pAllTrees[]`.
