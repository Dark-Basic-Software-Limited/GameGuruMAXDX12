...
// DOCDOC: generatehelpfromdocdoc = Set to 1 to generate new assetitinerary file every time, 2 to prevent CUSTOM_* being added to FPM (dev)
					...
					// DOCDOC: treeshadewrap = how much to LIFT the shaded side of distant-tree billboards, as a PERCENT (0-100). DEFAULT 0, bit-identical to the old rendering. A distant-tree billboard is a flat quad lit through a normal map baked off the real tree and deliberately exaggerated, so the tree still reads as three-dimensional from any angle; the side effect is that its shaded patches go very dark next to the smoothly-lit real mesh. This wraps the diffuse term (half-Lambert) so the shade lifts while the lit side and the shading PATTERN are left alone. 30-50 is a reasonable range; 100 is fully wrapped and starts to look flat.
