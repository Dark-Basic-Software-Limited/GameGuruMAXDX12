...
// DOCDOC: generatehelpfromdocdoc = Set to 1 to generate new assetitinerary file every time, 2 to prevent CUSTOM_* being added to FPM (dev)
					...
					// DOCDOC: lowvram = Low video memory preset, for cards with about 4GB. DEFAULT 0 (off - full DX12 visuals). Set 1 to trade visual reach for video memory so demos fit inside a 4GB card while holding 60 FPS. Members: grass draw distance cap (lowvramgrassdist), grass density (lowvramgrassdensity), shadow cascade/spot resolution capped at 1024, and SSR forced off. All are CAPS/clamps applied at the same points the level's own visuals are applied. See GameGuru Core/VRAM_FLOOR.md for the full floor analysis and what each knob buys.
