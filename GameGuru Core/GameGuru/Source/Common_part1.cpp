...
// DOCDOC: generatehelpfromdocdoc = Set to 1 to generate new assetitinerary file every time, 2 to prevent CUSTOM_* being added to FPM (dev)
					...
					// DOCDOC: envsolid = DEBUG RIG (#157). 1 replaces every read of the global env-probe cube with a flat magenta, covering both the specular reflection and the ambient term. Anything still showing structure is not coming from the cube texture. Default 0. Live equivalent: harness SET_ENVSOLID.
