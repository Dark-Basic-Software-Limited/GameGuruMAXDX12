...
// DOCDOC: generatehelpfromdocdoc = Set to 1 to generate new assetitinerary file every time, 2 to prevent CUSTOM_* being added to FPM (dev)
					...
					// DOCDOC: terrainbakeres / waterbakealpha = tuning for the GGMAX 3.25 "Terrain Bake" and "Water Bake" tick boxes in Graphics and Performance. These are NOT switches - the switches themselves are PER-LEVEL and saved in the level, unlike noterrain/nowater above, so a machine-wide override would give one control two owners. terrainbakeres is the baked texture size per terrain chunk in pixels per axis, DEFAULT 256, range 32-2048: it is the fidelity/video-memory dial, and cost grows with the SQUARE, so 512 is four times the memory of 256 (256 costs about 220 MB over 625 chunks, against the roughly 576 MB virtual-texture atlas the mode removes). Raise it if baked ground looks too soft underfoot, drop it on a card short of memory. Takes effect on the NEXT bake - untick and re-tick Terrain Bake to rebuild. waterbakealpha is how opaque the flat stand-in water plane is, as a PERCENT, DEFAULT 62: 100 is solid, low values let the ground show through.
