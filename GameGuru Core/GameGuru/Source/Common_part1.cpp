...
// DOCDOC: generatehelpfromdocdoc = Set to 1 to generate new assetitinerary file every time, 2 to prevent CUSTOM_* being added to FPM (dev)
					...
					// DOCDOC: noterrain / notrees / nograss / nowater = BRUTAL off-switches for weaker GPUs (GGMAX 2.94). DEFAULT 0 (everything on). Set 1 and that subsystem is not merely hidden - its elements never enter the scene, so they cost no culling, no Scene::Update transform/AABB work, no shadow casting, no virtual-texture feedback and no video memory. These are MACHINE-WIDE (all levels); the same four switches exist per-session as tick boxes in Graphics and Performance, and either one being on turns the subsystem off. noterrain removes the rendered terrain (the CPU heightmap stays, so ground-height queries and physics still work). notrees releases the tree pool, its shadow proxies and the per-type LOD assets. nograss removes every grass HairParticleSystem (the level's PAINTED grass map is untouched and will come back). nowater removes the ocean and, with it, the whole planar-reflection pass - measured at 4.37 ms of a 10.06 ms GPU frame on a water level, which makes it the single biggest of the four.
					// DOCDOC: terrainbakeres / waterbakealpha = tuning for the GGMAX 3.25 "Terrain Bake" and "Water Bake" tick boxes in Graphics and Performance. These are NOT switches - the switches themselves are PER-LEVEL and saved in the level, unlike noterrain/nowater above, so a machine-wide override would give one control two owners. terrainbakeres is the FAR baked texture size per terrain chunk in pixels per axis, DEFAULT 256, range 32-8192, and terrainbakeresnear is the NEAR size, DEFAULT 4096, used for the chunks carrying the most placed objects (the play area) with terrainbakenearbudget, DEFAULT 512, capping the near tier in MB - chunks are promoted densest-first until that budget is spent, so distant scenery stays cheap while the ground you walk on gets sixteen times the texel density. Together: it is the fidelity/video-memory dial, and cost grows with the SQUARE. The baked chunk textures are BC1 compressed, which is what makes 1024 affordable - measured over 625 chunks the whole bake costs about 84 MB at 256, 142 MB at 512 and 377 MB at 1024, against the roughly 576 MB virtual-texture atlas the mode removes, so even the top setting is a net saving. Frame rate does NOT change across that range; this dial buys sharpness with video memory and nothing else. Drop it to 512 or 256 on a card short of memory. Takes effect on the NEXT bake - untick and re-tick Terrain Bake to rebuild. Takes effect on the NEXT bake - untick and re-tick Terrain Bake to rebuild. waterbakealpha is how opaque the flat stand-in water plane is, as a PERCENT, DEFAULT 62: 100 is solid, low values let the ground show through.
					t.tryfield_s = "terrainbakeres"; if (t.field_s == t.tryfield_s) { extern int gg_terrain_bake_res; if (t.value1 >= 32 && t.value1 <= 8192) gg_terrain_bake_res = t.value1; }
					...
					// DOCDOC: objectculldist = how far away objects are still drawn, in world units where 1 unit is an inch (GGMAX 3.11). DEFAULT 0 = off, draw everything however far away it is. Set 20000 (about 500 metres) and anything beyond that simply is not drawn; set 8000 (200 metres) for a much more aggressive cut. Objects do not pop out - each one dissolves away over a distance of roughly its own size - and they come straight back when you raise the number again. This is the bluntest instrument in the panel and on a big outdoor level it is also the most effective, because it removes the draw calls, the triangles and the shadow casting all at once. Trees are not affected: they run their own distance system and keep it.
					t.tryfield_s = "objectculldist"; if (t.field_s == t.tryfield_s) { extern void GGSetObjectCullDist(int); GGSetObjectCullDist(t.value1); }

					// DOCDOC: texturedivide = load every texture at a fraction of its authored size (GGMAX 3.12). DEFAULT 1 = full size, as authored. 2 loads a 1024x1024 texture as 512x512, 4 loads it as 256x256. This is the setting to reach for on a card that is short of video memory or short of memory BANDWIDTH, which is most older cards: it cuts texture memory roughly four-fold at 2 and sixteen-fold at 4, and it makes every texture fetch cheaper as well as smaller. The cost is exactly what you would expect - surfaces get softer the closer you stand to them. It applies as levels LOAD, so change it and then load your level; textures already in memory keep the size they were loaded at. A handful of oddly sized textures keep more detail than asked for, because shrinking them would produce a size the graphics card cannot represent.
					t.tryfield_s = "texturedivide"; if (t.field_s == t.tryfield_s) { extern void GGSetTextureDivide(int); GGSetTextureDivide(t.value1); }
					...
					// DOCDOC: treeshadewrap = how much to LIFT the shaded side of distant-tree billboards, as a PERCENT (0-100). DEFAULT 50. Set 0 for the pre-3.07 rendering, which is bit-identical and so makes a clean A/B. A distant-tree billboard is a flat quad lit through a normal map baked off the real tree and deliberately exaggerated, so the tree still reads as three-dimensional from any angle; the side effect is that its shaded patches go very dark next to the smoothly-lit real mesh. This wraps the diffuse term (half-Lambert) so the shade lifts while the lit side and the shading PATTERN are left alone. 50 is the shipped value and the top of the useful range - past about 60 the wrap eats the shading pattern rather than lifting it and the tree flattens toward a card.
					t.tryfield_s = "treeshadewrap"; if (t.field_s == t.tryfield_s) { extern void GGSetTreeShadeWrap(int); GGSetTreeShadeWrap(t.value1); }
					...
					// DOCDOC: probeparallax = How the environment-probe parallax correction behaves. DEFAULT 3 = full float precision AND no parallax correction against level-sized probe boxes, which is what DX11 did; parallax against a box enclosing the whole level only drags the reflected horizon by the surface's distance from the map origin, making the look position-dependent for no physical gain. 1 = float precision with the parallax kept (the precision-only fix). 0 restores the stock half-precision math, which overflows fp16 (max 65504) for any probe box bigger than ~37800 units and puts circular reflection artifacts on every shiny surface - the global probe box is 50000 units, so 0 is the DEFECT setting and exists only for A/B. 2 = float plus a magenta overlay marking every pixel the stock math would have broken.
